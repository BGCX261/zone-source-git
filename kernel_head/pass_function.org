...
- 要先判断wait->task_list为空的时候才把wait加入队列。为什么在sleep_on里不用呢?因为
  prepare_to_wait的应用场合不同，prepare_to_wait会放在一个循环里重复调用，但是finish_wait不
  会被放到循环里，看看__wait_event就知道了。
- 虽然在is_sync_wait里会检查wait是否为空，但进入prepare_to_wait是肯定不会为空的，所以
  is_sync_wait做了多余的事情。
** #define is_sync_wait(wait)	(!(wait) || ((wait)->task))
*** include/linux/wait.h:
- 有一段注释：Used to distinguish between sync and async io wait context: sync i/o
  typically specifies a NULL wait queue entry or a wait queue entry bound to a task
  (current task) to wake up. aio specifies a wait queue entry with an async notification
...
*** kernel/wait.c:
- 用了list_empty_careful，为什么呢？只能用于调用list_del_init的情况，因为list_del_init里调
  用了INIT_LIST_HEAD
...
- 如何通过一个task_t来获得一个运行队列:从task_t里的thread_info里的CPU来找到task是在哪一个
...
** not function
*** 12.1.1
- 注意什么是文件模型(file model)的概念
- 从图12-2可以看出,一个磁盘文件可以和一个superblock object关联,之后superblock object再和一
  个inode object关联,或是一个磁盘文件直接与一个inode object关联,注意都是一对一的关系.
- page cache也是disk cache?ulk:Beside the dentry cache and the inode cache, Linux uses
  other disk caches. The most important one,called the page cache, is described in detail
  in Chapter 15.  superblock object和file object是没有cache的?
- We said earlier that the VFS is a layer between application programs and specific
  filesystems.
- superblock object是与一个已被挂载的文件系统相关联的,如果是一个基于磁盘的文件系统的话,那么
  superblock object对应保存在磁盘的filesystem control block.  ulk:Stores information
  concerning a mounted filesystem. For disk-based filesystems, this object usually
  corresponds to a filesystem control block stored on disk.
- 按照ulk的说法,file.f_count在两种情况下会增加,一个与CLONE_FILE有关,一个与dup()有关.ulk:The
  f_count field of the file object is a reference counter: it counts the number of
  processes that are using the file object (remember however that lightweight processes
  created with the CLONE_FILES flag share the table that identifies the open files, thus
  they use the same file objects). The counter is also increased when the file object is
  used by the kernel itselffor instance, when the object is inserted in a list, or when a
  dup( ) system call has been issued.
- dentry object也有in-use,unused或negative state 3种状态.
- 所有的dentry object都放到一个hash表里,通过一个给定的文件名和目录来快速查找
- dentry object也用最近最少使用来组织以方便回收.
- negative dentry是怎么产生的和怎么被回收的:An "in use" dentry object may become
  "negative" when the last hard link to the corresponding file is deleted. In this case,
  the dentry object is moved into the LRU list of unused dentries. Each time the kernel
  shrinks the dentry cache, negative dentries move toward the tail of the LRU list so that
  they are gradually freed.
- 进程描述符和文件相关的两个域是struct fs_struct类型的fs域和struct file_struct类型的files
  域.
- ulk:thanks to the dup( ) , dup2( ) , and fcntl( )system calls, two file descriptors may
  refer to the same opened filethat is, two elements of the array could point to the same
  file object. dup()产生的是指向同一个file object的
- 什么是文件系统的挂载点(mount point)和什么是一个已挂载文件系统的孩子.
- 什么是进程的namespace:In a traditional Unix system, there is only one tree of mounted
  filesystems: starting from the system's root filesystem, each process can potentially
  access every file in a mounted filesystem by specifying the proper pathname. In this
  respect, Linux 2.6 is more refined: every process might have its own tree of mounted
  filesystemsthe so-called namespace of the process.
- ulk:The new namespace is then inherited by children processes if the parent creates them
  without the CLONE_NEWNS flag.
- ulk里所说的filesystem指的是在/dev/目录下的块设备文件,而不是指ext2,fat之类的,ulk12.4.2开
  头的说明可以看出.
- Linux is different: it is possible to mount the same filesystem several times. Of
  course, if a filesystem is mounted n times, its root directory can be accessed through n
  mount points, one per mount operation. Although the same filesystem can be accessed by
  using different mount points, it is really unique. Thus, there is only one superblock
  object for all of them, no matter of how many times it has been mounted.
  
  可以多次被挂载,这时可以从这个挂载点访问,但只有一个superblock object.所以文件系统是唯一的.
- 不同的filesytem可以挂载到同一个挂载点,形式同stack
- 从ulk的描述来看,struct vfsmount是与一个已挂载的filesystem相关联的,是一对一的关系.ulk:For
  each mount operation, the kernel must save in memory the mount point and the mount
  flags, as well as the relationships between the filesystem to be mounted and the other
  mounted filesystems. Such information is stored in a mounted filesystem descriptor of
  type vfsmount .
- 要查找一个vfsmount可以在一个hash表里查找;
  
  属于同一个namespace的filesystem有一个链表把它们链起来.
  
  一个filesystem的所有子filesystem有一个链表把它们链起来.

** long do_mount(char * dev_name, char * dir_name, char *type_page, unsigned long flags, void *data_page)
*** fs/namespace.c:
- 这个函数是sys_mount()调用用来挂载filesystem的.
- 开始把MS_NOSUID,MS_NODEV,MS_NOEXEC转成MNT_NOSUID,MNT_NODEV,MNT_NOEXEC,分别是禁止setuid和
  setgid,禁止访问块设置文件,不允许执行文件,为什么这3个那么特殊呢?
- 找到路径之后开始处理挂载的事了,这个处理要看传入的flags标志了
- 若标志是MS_REMOUNT,那么一般是想用来修改superblock的s_flags域和mnt_flags域.调用
  do_remount()来处理
- 若标志是MS_BIND,那么表示用户想把一个目录挂载到另一个目录上.对应mount命令的bind参数.
** static int do_remount(struct nameidata *nd, int flags, int mnt_flags, void *data)
*** fs/super.c:
- 这个函数做一些检查之后就调用do_remount_sb()
** int do_remount_sb(struct super_block *sb, int flags, void *data, int force)
*** fs/super.c:
- 若传入的flags里没有MS_RDONLY但是这个块设备是只读的,那么就要返回不能重新设置flags,而不是
  把MS_RDONLY给设置上再往下执行.
  
  通过调用bdev_read_only()来查看block_device.bd_part.policy(若block_device是一个分区)或
  block_device.bd_disk.policy(若block_device是一个磁盘)来判断,若为1表示为只读.
  
  block_device的这些policy是什么时候建立的呢?是根据什么建立的呢?

- 接着调用shrink_dcache_sb()来为superblock shrink dcache
** void shrink_dcache_sb(struct super_block * sb)
*** fs/dcache.c:
- 注释:Shrink the dcache for the specified super block. This is used to free the dcache
  before unmounting a file system.所以有两点:1.shrink指定superblock的dcache,2,在unmount前
  调用来free dcache.
- 在repeat这个标签之前主要是扫描整个dentry_unused这个链表,把找到的每一个d_sb为指定
  superblock的dentry重新插入到dentry_unused的表头,所以最后的结果是所有d_sb为指定superblock
  的dentry都在dentry_unused表前头.
- 下半部分就是删除dentry了
  
  删除dentry是不一定的,但是一定会把d_sb为指定的superblock的dentry从dentry_unused里删除
  
  若dentry.d_count不为0就不可以删除,那么什么时候会把这种dentry删除掉呢?
- 调用prune_one_dentry()做最后的释放.
- 因为在prune_one_dentry()里调用的dentry_iput()里会释放dcache_lock,所以在这个函数里不一定
  能从头到尾都持有dcache_lock这个锁,所以在这个空档时间内有可能有一dentry插入到了
  dentry_unused表头,就因为这个原因在这个函数的第二个while里要判断一下dentry.d_sb是不是指定
  的superblock
  #+BEGIN_EXAMPLE
		if (dentry->d_sb != sb)
			continue;
...
*** fs/dcache.c:
- 这个函数只有两个地方调用:prune_dcache(),shrink_dcache_sb()
- 注释:Throw away a dentry - free the inode, dput the parent.This requires that the LRU
  list has already been removed.Called with dcache_lock, drops it and then regains.

  dentry一定是已经从dentry_unused里删除的.
- 调用__d_drop()把dentry从hash表里删除,同时要设置DCACHE_UNHASHED.
- dentry.d_child:For directories, pointers for the list of directory dentries in the same
  parent directory.所以在这个函数里要把dentry.d_child链表里删除.
- dentry.d_alias:Pointers for the list of dentries associated with the same inode (alias).
  
  这个函数调用了dentry_iput()来释放该dentry所关联的inode.这个所谓的释放就是设置
  dentry.d_inode为空而已,并没有把关联的inode从disk里删除
  
  同时还要把dentry从dentry.alias链表里删除.
- 因为取消了一个dentry对inode的引用,所以要调用iput()来减inode的引用计数,有可能会把inode释
  放掉.
- 因为一个目录被删掉了,所以该目录的父目录的引用计数就要减一.那么是不是说创建一个文件的
  dentry时,它的父目录和父目录...的dentry也要创建呢?
  #+BEGIN_EXAMPLE
	if (parent != dentry)
		dput(parent);
  #+END_EXAMPLE 
...
*** fs/namei.c:
- ulk:Pathname lookup has to be done inside the namespace of the process that issued the
  system call. The same pathname used by two processes with different namespaces may
  specify different files.
- ulk:Because the dentry object and the mounted filesystem object returned by the
  path_lookup( ) function in the nameidata structure represent the result of a lookup
  operation, both objects should not be freed until the caller of path_lookup( ) finishes
  using them. Therefore, path_lookup( ) increases the usage counters of both objects. If
  the caller wants to release these objects, it invokes the path_release( ) function
  passing as parameter the address of a nameidata structure. 因为nameidata.dentry和
  nameidata.mnt会增加dentry object和filesystem object的计数器.
- 为什么开始要设置nameidata.last_type为LAST_ROOT呢?last_type表示:Type of last component of
  the pathname (used when the LOOKUP_PARENT flag is set)
- nameidata.flags是查找过程中要用的标志.
- nameidata.depth:Current level of symbolic link nesting (see below); it must be smaller
  than 6.所以一开始要设置为0.
- fs_struct.altroot是模拟根目录,LOOKUP_NOALT:LOOKUP_NOALT Do not consider the emulated
  root directory (useless in the 80x86 architecture).
- 接下来就是设置nameidata.mnt和nameidata.dentry了,设置之后的结果有3种:
  
  1. 分别为模拟文件系统和模拟根目录,在路径是绝对路径的情况下且有模拟根目录且没有设置LOOKUP_NOALT.
  
  2. 分别是进程的根目录(fs_struct.root)和进程的根文件系统(fs_struct.rootmnt).在路径 _不是_
     绝对路径的情况下且 _没有_ 模拟根目录或 _有_ 设置LOOKUP_NOALT.

  3. 分别是当前目录和当前目录文件系统
- task_struct.total_link_count:the total_link_count field in the descriptor of the current
  process keeps track of how many symbolic links (even nonnested) were followed in the
  original lookup operation.If this counter reaches the value 40, the lookup operation
  aborts. Without this counter, a malicious user could create a pathological pathname
  including many consecutive symbolic links that freeze the kernel in a very long lookup
  operation.
- 调用link_path_walk()来查找
** int fastcall link_path_walk(const char *name, struct nameidata *nd)
*** fs/namei.c:
- 主要是调用__link_path_walk()
** static fastcall int __link_path_walk(const char * name, struct nameidata *nd)
*** fs/namei.c:
- 把开头连续的/给删掉,用cd //home的确可以进入/home目录
- ulk:In the nameidata data structure,the dentry and mnt fields point to the objects
  relative to the last resolved component of the original pathname.
- 因为在path_lookup()里根据路径处理了nameidata.dentry,所以处理完路径名开头的/之后的第一个
  文件名就对应nameidata.dentry了
  #+BEGIN_EXAMPLE
	while (*name=='/')
		name++;
	if (!*name)
...
- 下面的代码有两个作用:
  ...
  1, 把最前的那个路径component给取出来,计算它的hash值以用来在dentry cache hash里找出dentry
  object. 总之一句话就是初始化了struct qstr
  
  2, 把name的指针向后移到了下一个component里去了
  #+BEGIN_EXAMPLE
		this.name = name;
...
- 接下来又是去连续/了
  ...
  #+END_EXAMPLE
- 接下来就是处理为'.'和'..'的情况,用follow_dotdot()来处理'..'.
  #+BEGIN_EXAMPLE
  ...
  注意上面的代码switch中有continue,这个continue不是作用于switch的,而是作用于外层的for()循
  环的.这说明了如果是一个'.'或'..'的话那么就开始解析下一个部分.注意有fallthrough的.
- 下面的会重新生成hash值,为什么要这样做呢?
  #+BEGIN_EXAMPLE
  ...
  #+END_EXAMPLE
- 设置LOOKUP_CONTINUE有什么用呢?LOOKUP_CONTINUE:There are still filenames to be examined
  in the pathname.
  #+BEGIN_EXAMPLE
		nd->flags |= LOOKUP_CONTINUE;
  #+END_EXAMPLE
- 调用do_lookup()作真正的查找.
- 找到的结果放到了next这个类型为struct path的结构体里.
- 是不是inode.i_op为空的时候就表示inode不是一个目录呢?
  #+BEGIN_EXAMPLE
		err = -ENOTDIR; 
		if (!inode->i_op)
			goto out_dput;
  #+END_EXAMPLE 
  不是的,如果是一个符号链接也不可以为空.
- 接下来就看看这个component是不是一个符号链接,i_operation.follow_link()不为空就表示是符号
  链接,这时调用do_follow_link()来处理.
- 若不是符号链接,就要把nameidata.mnt和maneidata.dentry修改为刚刚解析完的component的mnt和
  dentry
  #+BEGIN_EXAMPLE
		} else {
			dput(nd->dentry);
  ...
  #+END_EXAMPLE 
  其实是符号链接的时候这些东西应该已经在do_follow_link()里做了.
- 如果在这个时候发现解析完的这个component不是一个目录,就会返回ENOTDIR表示在路径的中间遇到
  了一个不是目录的component.
  #+BEGIN_EXAMPLE
		err = -ENOTDIR; 
		if (!inode->i_op->lookup)
			break;
  #+END_EXAMPLE 
  否则就continue到下一个component.
- 如果发现路径的最后一个字符是/,那么就设置LOOKUP_FOLLOW和LOOKUP_FOLLOW If the last
  component is a symbolic link, interpret (follow) it .LOOKUP_DIRECTORY. LOOKUP_DIRECTORY:
  The last component must be a directory.为什么要设置这两个标志?ulk:If the pathname has a
  trailing slash, it sets the LOOKUP_FOLLOW and LOOKUP_DIRECTORY flags in the lookup_flags
  local variable to force the last component to be interpreted by later functions as a
  directory name.
- 到了last_with_slashes就说明已经解析到了最后一个component了,注意只是解析到了,但是没有解析,因
  为last_with_slashes和last_comonent是在用最后一个component初始化类型为struct qstr的this之
  后就跳转的
  #+BEGIN_EXAMPLE
		this.name = name;
		c = *(const unsigned char *)name;
		...
		if (!*name)
			goto last_with_slashes;
  #+END_EXAMPLE
- 要清掉LOOKUP_CONTINUE,因为到这里已经是最后一个component了.  
 
  LOOKUP_CONTINUE There are still filenames to be examined in the pathname.
- 解析最后一个component的代码与之前解析其它的component基本一样.调用follow_dotdot(),调用
  do_lookup()调用do_follow_link()
- 为什么要做下的检查
  #+BEGIN_EXAMPLE
		err = -ENOENT;
		if (!inode)
			break;
  #+END_EXAMPLE 
  ulk:Checks whether nd->dentry->d_inode is NULL. This happens when there is no inode
  associated with the dentry object, usually because the pathname refers to a nonexistent
  file. In this case, the function returns the error code -ENOENT.
...
- LOOKUP_PARENT: Look up the directory that includes the last component of the pathname
  ...
  在很多情况下，查找操作的真正目的并不是路径名的最后一个分量，而是最后一个分量的前一个分量。
  例如，当文件被创建时，最后一个分量表示还不存在的文件的文件名，而路径名中的其余路径指定新
  链接必须插入的目录。因此，查找操作应当取回最后分量的前一个分量的目录项对象。另举一个例子，
  我们执行rm -f /foo/bar，这里路径名/foo/bar表示的文件bar拆分出来就包含从其目录foo中移去
  bar。因此，内核真正的兴趣在于访问文件目录foo 而不是bar。

  当查找操作必须解析的是包含路径名最后一个分量的目录而不是最后一个分量本身时，就使用
  LOOKUP_PARENT标志。当LOOKUP_PARENT标志被设置时，link_path_walk()函数也在nameidata数据结构
  中建立last和last_type字段。last字段存放路径名中的最后一个分量名。
  ...
  当整个路径名的查找操作开始时，LAST_ROOT标志是由path_lookup()设置的缺省值。如果路径名正好
  是“/”，则内核不改变last_type字段的初始值。last_type字段的其他值在LOOKUP_PARENT标志置位时
  由link_path_walk()设置

  LOOKUP_PARENT标志的值，如果被置位则执行如下操作：

  a. 把nd->last置为最后一个分量名。this就是最后一个分量
  #+BEGIN_EXAMPLE
		nd->last = this;
  #+END_EXAMPLE 
  ...
  可以看到，最后一个分量根本就没有被解释。因此，当函数终止时，nameidata数据结构的dentry和
  mnt字段指向最后一个分量所在目录对应的对象。
  
  执行到lookup_parent这里时最后一个component还是没有解析出它的dentry的.
  
  如果最所一个component是一个..,那么就要执行return_reval标签调用d_revalidate(),否则都是退
  出这个函数返回0.
- http://blog.chinaunix.net/uid-12567959-id-160997.html
  FS_REVAL_DOT标志（始终在目录项高速缓存中使“.”和“..”路径重新生效（针对网络文件系统））
- 到现在发生在这个函数里若使用break来跳出解析循环的,都是错误的情况
  #+BEGIN_EXAMPLE
  return_err:
	return err;
  #+END_EXAMPLE
- 如果最后一个component是一个.,那么退出这个函数返回0,表示没有错误.因为.的父录已经在
  nameidata里了.
  #+BEGIN_EXAMPLE
			case 1:
				goto return_reval;
...
*** fs/namei.c:
- vfsmount.mnt_parent: Points to the parent filesystem on which this filesystem is
  mounted.
- vfsmount.mnt_root: Points to the dentry of the root directory of this filesystem.
- dentry.d_mounted:d_mounted or directories, counter for the number of filesystems
  mounted on this dentry.
- 下面的代码对应ulk的这段话:If the last resolved directory is the process's root directory
  (nd->dentry is equal to current->fs->root and nd->mnt is equal to current->fs->rootmnt),
  then climbing is not allowed: it invokes follow_mount( ) on the last resolved component
  (see below) and continues with the next component.
...
- 接下来的代码对应ulk:If the last resolved directory is not the root directory of a
  mounted filesystem, then the function must simply climb to the parent directory: it sets
  nd->dentry to nd- >dentry->d_parent, invokes follow_mount( ) on the parent directory,
  and continues with the next component.
  #+BEGIN_EXAMPLE
  ...
  #+END_EXAMPLE
  只修改了nameidata.dentry为它自已的父目录.

- 接下来的代码对应ulk:If the last resolved directory is the root directory of the nd->mnt
  filesystem (nd- >dentry is equal to nd->mnt->mnt_root) and the nd->mnt filesystem is not
  mounted on top of another filesystem (nd->mnt is equal to nd->mnt->mnt_parent), then the
  nd- >mnt filesystem is usually[*] the namespace's root filesystem: in this case,
  climbing is impossible, thus invokes follow_mount( ) on the last resolved component (see
  below) and continues with the next component.
  #+BEGIN_EXAMPLE
...
- 接下来的代码对应ulk:If the last resolved directory is the root directory of the nd->mnt
  filesystem and the nd->mnt filesystem is mounted on top of another filesystem, a
  filesystem switch is required. So, the function sets nd->dentry to
  nd->mnt->mnt_mountpoint , and nd->mnt to nd->mnt->mnt_parent, then restarts step 5g
  (recall that several filesystems can be mounted on the same mount point).
  #+BEGIN_EXAMPLE
		mntget(parent);
		...
		nd->mnt = parent;
  #+END_EXAMPLE
- 注意函数是用了一个死循环来处理'..'的,所以只有最后一种情况下还要重新做解析.为什么?因为最
  后一种情况只是做了filesystem的切换,并没有真正处理'..'.所以要再一次.
- 最终的结果就是修改了nameidata.dentry和nameidata.mnt.
  
  若要切换文件系统就修改nameidata.mnt.
  
  若没有上级目录就不能候改nameidata.dentry.
- 无论什么情况,都会调用follow_mount().
** static void follow_mount(struct vfsmount **mnt, struct dentry **dentry)
*** fs/namei.c:
- 若有多个filesystem挂载到某个filesystem的同一个dentry里,那么这些filesystem的
  vfsmount.mnt_root是不是都是相等的呢?
  
  除了vfsmount.mnt_root之外,还有vfsmount.mnt_mountpoint:mnt_mountpoint Points to the
  dentry of the mount point directory where the filesystem is mounted.
  
  可见是不一样的.

- 总之,传进来的参数dentry一定是在mnt下的dentry.

- 调用lookup_mnt()来找一个挂载到指定的filesystem里的指定的dentry的filesystem.也就是要找到
  一个filesystem的mnt_parent等于参数mnt,mnt_mountpoint等于参数dentry.
  
  又因为在follow_mount()里除了第一次调用lookup_mnt()外,其它的调用的第二个参数都是mnt_root.
  
  所以可以总结出:follow_mount()的这个循环里循环处理的是多个filesystem以堆栈的形式挂载在前
  一个filesystem的根目录里.而不是多个filesystem挂载到某个filesystem的同一个dentry下.
- 但是若dentry.d_mounted大于1,就是说有多个filesystem桂载到同一个dentry下时,follow_mount()
  只能处理一个,这样合理吗?如果合理的话,解释应该是这样子的:就是挂载到同一个dentry下的多个
  filesystem只能访问最后一个被挂载的filesystem的数据,这样的话那么lookup_mnt()返回的就是那
  个可以访问的,而不是其它不可以访问的,那么lookup_mnt()又是如何保证这点的呢?
- 结合follow_dotdot()来看,如果是下面的那种情况
  #+BEGIN_EXAMPLE
		if (nd->dentry == current->fs->root &&
		    nd->mnt == current->fs->rootmnt) {
                        read_unlock(&current->fs->lock);
  ...
  #+END_EXAMPLE 
  那么这个'..'不是真正的有效.因为最多dentry.mnt_mounted大于1而转到其它的filesystem的根目录
  里,而不会再进入上一级目录.
  ...
  #+END_EXAMPLE  
  结合follow_mount()来看又是什么情况呢?一个目录A的父目录可以被挂载上其它的filesystem吗?如果
  可以的话好像是有问题的,因为父目录挂载了其它的filesystem了,那么不可以通过这个所谓的父目录
  来访问另一个filesystem的目录A的.
** static int do_lookup(struct nameidata *nd, struct qstr *name, struct path *path)
*** fs/namei.c:
- 这个函数就是返回name.name对应的的dentry.先是在dcache里找.
- 按照ulk的说法参数nd.dentry是name.name这个成分的父目录:Invokes do_lookup( ) to derive the
  dentry object associated with a given parent directory (nd->dentry) and filename (the
  pathname component being resolved).
  
  但是从调用do_lookup()的__link_path_walk()却看不出来.假设在path_lookup()里一直
  到__link_path_walk()都看不出是父目录.
  
  好像在name为绝对路径的时候才会正确,因为在path_lookup()里若name为绝对路径,那每
  nameidata.dentry就是模拟根目录或进程namespace的根目录,转而调用link_path_walk(),再转而调
  用__link_path_walk(),在__link_path_walk()里一开始就把/给删除不作解析,所以紧接下来的那个
  component就是nameidata.dentry的子目录,因为nameidata.dentry是根目录的dentry.
  
  但如果不是绝对路径的话好像就不正确了.突然想明白了,还是正确的,就是你在当前目录A,想进入它
  的子目录B,这时你是输入cd B,而不是cd A/B,所以还是正确的.

- 先调用__d_lookup()在dcache里找.返回0表示在dcache里没有找到.
- d_revalidate:Determines whether the dentry object is still valid before using it for
  translating a file pathname. The default VFS function does nothing, although network
  filesystems may specify their own functions.
- 若在dcache里没有找到,那么就要调用real_lookup()来建一个了.
- 找到的结果放到参数path里,这个struct path结构体只有mnt和dentry这两个成员.
- 因为找到的dentry可能被挂载上了其它filesystem,所以要调用__follow_mount()来找出最后一次挂
  载到这个dentry的filesystem.并修改path.
** struct dentry * __d_lookup(struct dentry * parent, struct qstr * name)
*** fs/dcache.c:
- 用dentry cache的hash表来找出要找的dentry.
- 注意一下d_hash()的使用,第一个参数是要找的dentry的父目录,ulk:The dentry parameter
  identifies the directory containing the component. The name parameter points to a
  structure containing both the pathname component to be looked up and the value produced
  by the hash function. 通过d_hash()就可以找到要找的dentry对应的hash表项,之后就可以通过
  hash链表来查找了.
  
  关于dentry cache的 hash表.整个hash链表的表头是static struct hlist_head
  *dentry_hashtable;dentry_hashtable会在dcache_init()里调用alloc_large_system_hash()来分配
  空间,这个空间就是以struct hlist_head *为类型的的空间,可以被过d_hash()的代码程返回值可以
  看出来
  #+BEGIN_EXAMPLE
	return dentry_hashtable + (hash & D_HASHMASK);
  #+END_EXAMPLE 
- d_hash()返回的就为要找的dentry所在的hash链表的表头,接下来就用hlist_for_each_rcu()来遍历
  整个链表来找,找到的条件为
...
*** fs/namei.c:
- 要重新做一次在dcache里找,因为在获取信号量的时候有可能会被其它进程建了一个dentry,这个是注
  释上说的
  #+BEGIN_EXAMPLE
	/*
  ...
  #+END_EXAMPLE
- 若在dcache里还找不到,那么就用d_alloc()分配一个dentry object,再用inode.i_op.lookup()从磁
  盘里读出来.
  
  inode_operation.lookup()Searches a directory for an inode corresponding to the filename
  included in a dentry object.
** static inline int do_follow_link(struct path *path, struct nameidata *nd)
*** fs/namei.c:
- 这个函数是用来查找路径时处理符号链接的.
- the total_link_count field in the descriptor of the current process keeps track of how
  many symbolic links (even nonnested) were followed in the original lookup operation. If
  this counter reaches the value 40, the lookup operation aborts. Without this counter, a
  malicious user could create a pathological pathname including many consecutive symbolic
  links that freeze the kernel in a very long lookup operation.
  
  task_struct.total_link_count就算没有嵌套也要增加.
- 如何判断一个文件是符号链接呢?若inode的follow_link()方法不为空就表示是
- 若task_struct.link_count大于或等于5就返回,不用再解析,link_count是用来避免在同一个符号链
  接里无限follow的.
- 接下来就是检查total_link_count.
- 检查完link_count和total_link_count之后就是增加它们.
- 还要增加nameidata.depth,depth Current level of symbolic link nesting (see below); it
  must be smaller than 6
  
  那么nameidata.depth是在哪里检查的呢?
- 主要是调用__do_follow_link()来处理.
** static inline int __do_follow_link(struct path *path, struct nameidata *nd)
*** fs/namei.c:
- 调用touch_atime()来修改符号链接的inode的访问时间,为什么要修改这个昵?是不是因为要读取里面
  的内容呢?
  
  而在查找路径时有没有修改目录的访问时间呢?

- 通过nd_set_link()可以看出,nameidata.saved_names是一个数组,这个数组的大小是
  nameidata.depth的最大值,用来保存每一次嵌套符号链接时的路径.

- 接收到path.dentry参数,是正需要被解析的符号链接.
- 接下来就是调用文件系统特定的follow_link()函数,这个函数里会做一件事,就是把符号链接文件里
  的内容取出来放是nameidata.saved_names数组里.
- 调用nd_get_link()把最外层的路径取出来解析
  #+BEGIN_EXAMPLE
		char *s = nd_get_link(nd);
  #+END_EXAMPLE
- 调用__vfs_follow_link()
- 调用inode的put_link()的方法来释放调用follow_link()时分配的空间.
** static inline int __vfs_follow_link(struct nameidata *nd, const char *link)
*** fs/namei.c:
- 若符号链接解析出来的路径名是一个绝对路径,那么就调用path_release()转而调用dput()和
  mntput()
- 这里也调用了link_path_walk(),这里调用的link_path_walk()只是把符号链接里的内容给解析了,解
  析完之后那么最后一个component就放在nameidata里,并把nameidata返回给调用
  do_follow_link()的__link_path_walk().
** asmlinkage long sys_open(const char __user * filename, int flags, int mode)
*** fs/namei.c:
- 调用getname()从用户态空间来获取要打开的文件路径.
- 调用get_unused_fd()在current.files.fd数组里找出一个空闲的文件描述符.
- 接下来就是调用filp_open()
** char * getname(const char __user * filename)
*** fs/namei.c:
- 先调用__getname()来分配内核空间来存放路径名,__getname()是一个调用kmem_cache_alloc()的宏,
  使用的是names_cache cache.这个names_cache创建的时候指定的object大小的PATH_MAX,也就是4096
  这字节.
- 分配好空间后就调用do_getname().
** static inline int do_getname(const char __user *filename, char *page)
*** fs/namei.c:
- 为什么要调用segment_eq来判断呢?
- 主要是调用strncpy_from_user()来拷贝路径名.
** int get_unused_fd(void)
*** fs/open.c:
- 主要是调用find_next_zero_bit()找出一个空闲的fd.这个函数用到了
  files.open_fds.fds_bits,files.max.fdset,files.next_fd.
- 找出一个空闲的fd之后就要调用expand_files()判断一下是否要扩展fd数组,若需要就同时扩展一下.
- expand_files()返回1表示需要扩展但是扩展过程中遇到了阻塞了.
- expand_files()成功后就表明open_fds里一定有一个可以设置的位了,那么就会调用FD_SET()设置相
  应的files.open_fds位.但是为什么要清files.close_on_exec的位呢?close_on_exec里的位没有被使
  用不是本应该是清除的吗?
** int expand_files(struct files_struct *files, int nr)
*** fs/file.c:
- 注释说是返回<0的值表示有错,返回0表示做了扩展,返回1表示要扩展但扩展过程被阻塞了.
- files.max_fds是指类型为struct files *的files.fd数组的大小,
  
  files.max_fdset是指向当前打开的文件的文件描述符的位图的最大位数,就是files.open_fds.
  
  files.max_fds和files.max_fdset还是不同的,不一定一样,因为max_fdset指的是最大位数,而不定所
  有位对应的文件描述符都被使用.
- 调用expand_fdset()扩展fdset位图
- files.fd数组也不是时刻时刻都填满的,也使用了expand_fd_array(),
** static int expand_fdset(struct files_struct *files, int nr)
*** fs/file.c:
- nr是当前已被使用的fd,那么它是不是最大的呢?从get_unused_fd()来看,这个要取决于
  find_next_zero_bit().
- 先判断files.max_fdset和nr有没有超过NR_OPEN
- 接下来就是找出合适的将要扩展的大小,
  #+BEGIN_EXAMPLE
	do {
		if (nfds < (PAGE_SIZE * 8))
  ...
  这个增加的方法有点意思,1.就是被扩展之后的大小最少是一个页的大小,2.若扩展之前的大小大于一
  个页,那么就是翻倍的了,这可能是为了减少内存的碎片吧.
- 计算好需要多少个位之后,就可以把原来的位设置放到一个新分配的空间里.
  
  注意要同时处理files.open_fds和files.close_on_exec这两个成员,为什么呢?而且这两个成员的空间
  大小要一样.因为可能进程打开的所有文件都要求在执行exec()时关闭文件,所以这两个成员的空间至
  少要一样.
  ...
  #+END_EXAMPLE 
  什么情况下i会等于0呢?注释说是还没有被初始化,是不是出现竞争了呢?就是还没有初始化进程就打
  开文件了.
  
  接下来就是交换原来空间和新空间的地址,调用xchg(),这个函数是用汇编实现的.
  
  到这里已经扩展成功了,可以返回了,
- 因为调用alloc_fdset()分配新的空间时可能会休眠,所以要分配后要判断是不是有其它的进程进行了
  扩展.
  #+BEGIN_EXAMPLE
...
*** fs/file.c:
- 处理的方法与expand_fdset()差不多.
- 也会先判断有没有超过NR_OPEN,
- 也会计算应该扩展到多大
- 也是先分配一个合适的空间,然后把原来的内容拷贝到新的空间上,并把原来的空间给释放掉
- 也处理了分配新空间时因休眠而被其它的进程给扩展了.
- 有点不同的是有一个NR_OPEN_DEFAULT指定空间至少要为这个值.
- 计算也是至少一个页,且会翻倍.
** struct file *filp_open(const char * filename, int flags, int mode)
*** fs/open.c:
- 把O_RDONLY,O_WRONLY和O_RDWR转成合适的位,因为与sys_open()的不一样,有注释
  #+BEGIN_EXAMPLE
...
- 现在是下面的代码了,有点不明白
  ...
  为什么是O_TRUNC时要给以写的访问权限呢?
  
  apue:O_TRUNC:O_TRUNC 如果此文件存在,而且为只读或只写成功打开,则将其长度截短为 0.
  
  APUE:为了在open函数中对一个文件指定O_TRUNC标志,必须对该文件具有写许可权.
  
  通过APUE的解释,之所以要给写的访问权限,可能是为了检查进程是否对该文件有写的权限,而不是真
  正打开来写的.
- 然后调用open_namei()
** int open_namei(const char * pathname, int flag, int mode, struct nameidata *nd)
*** fs/namei.c:
- 调用ACC_MODE()宏来找出访问权限的模式
  #+BEGIN_EXAMPLE
#define ACC_MODE(x) ("\000\004\002\006"[(x)&O_ACCMODE])
  #+END_EXAMPLE 
  这个宏有点意思,
  
  如果flag为no permissions,那么返回0
  
  如果为read permission,那么返回4
  
  如果为write permission,那么返回2
  
  若为read/write permissions,那么返回6
  
  从这里可以知道O_RDONLY,O_WRONLY,O_RDWR只能为一个,调用open()时若指定了多个,那么最后的权限
  若不是O_RDWR就是O_WRONLY,要不就是O_RDONLY
  
  为什么要转换这些权限数字呢?应该是把这些访问权限转为文件访问权限的形式,就要chmod里使用的
  数字一样
  #+BEGIN_EXAMPLE
#define MAY_EXEC 1
#define MAY_WRITE 2
...
#define MAY_APPEND 8
  ...
  至于为什么O_RDWR是6呢?可能是MAY_WRITE + MAY_READ吧

- 接着转换O_APPEND为MAY_APPEND
- 现在看回sys_open()函数,第三个参数是在创建文件时才用的.
  
  APUE:O_CREAT 若此文件不存在则创建它。使用此选择项时,需同时说明第三个参数 mode,用其说明该
  新文件的存取许可权位。
- 指定nameidata.intent.open.flags为转换后的flags,并把创建文件的模式保存到
  nameidata.intent.open.create_mode.
  
  mode应该是S_IRUSR,S_IWUSR,S_IRGRP之类的.
- 如果没有指定O_CREAT来创建文件,那么就调用path_lookup()来查找路径.
  
  为什么要指定LOOKUP_OPEN来查找路径.
  
  调用path_lookup()的标志好像和ulk说的不一样,ulk:If O_CREAT is not set in the access mode
  flags, starts the lookup operation with the LOOKUP_PARENT flag not set and the
  LOOKUP_OPEN flag set. Moreover, the LOOKUP_FOLLOW flag is set only if O_NOFOLLOW is
  cleared, while the LOOKUP_DIRECTORY flag is set only if the O_DIRECTORY flag is set.
  ...
  其实是一样的,因为使用了lookup_flags()来转换O_~到LOOKUP_~.

- 如果查找成功就调用may_open()
- 若设置O_CREAT,那么调用path_lookup()时要指定LOOKUP_PARENT|LOOKUP_OPEN|LOOKUP_CREATE,为什
  么不指定LOOKUP_DIRECTORY呢?不能指定LOOKUP_DIRECTORY的,因为最后一个component是将要创建的
  文件名.
  
  指定LOOKUP_CREATE有什么用呢?
  
  nameidata.last_type的这个last指的不是当前的nameidata.dentry的类型,last_type在
  LOOKUP_PARENT设置时才会设置,这时last_type是最后一个component的类型,而nameidata.dentry是
  最后一个component的父目录.
- 调用__lookup_hash()来获得dentry object
- 获得dentry object之后若发现没有对应的inode object,那么就要调用vfs_create()来创建一个
  inode
  #+BEGIN_EXAMPLE
...
- O_EXCL: With O_CREAT, fail if the file already exists
  ...
  注意到这里时path.dentry是最后一个component的.
- 接下调用__follow_mount()是什么意思呢?path.dentry不是一个普通文件的dentry吗?可以用一个普
  通文件的dentry来调用__follow_mount()吗?
- 这个函数的作用是不是查找要找的文件的dentry放到参数nd.dentry里返回呢?
- 执行到下面的这一段话,最后一个component怎么可能是一个符号链接呢?
  #+BEGIN_EXAMPLE
...
- 这个函数有多要goto标签,
  ...
  do_last应该处理最后一个component用的,
  
  ok应该是成功时找到合适的dentry,放到了nameidata.dentry.
  
  exit_dupt应该是失败时要先释放path.dentry时用的
  
  exit应该也是失败时要用的.
  
  do_link:应该是在处理最后一个component时发现它是一个符号链接时用的.
  
  所以成功的话一定是到ok标签的,之后就会调用may_open()
- 若找到dentry,但是文件已被删除的情况下,先把inode给创建了,到调用may_open()之前要把acc_mode
  给清掉,为什么呢?
** int may_open(struct nameidata *nd, int acc_mode, int flag)
*** fs/namei.c:
- 为什么多一个FMODE_READ/FMODE_WRITE来表示文件的读写权限呢?

  在open_namei()传进来的flag都是以O_为开头的标志,不过是经过在filp_open()做加1处理的了.
  
  通过这个加1处理之后1就表示了读权限,因为原来的O_RDONLY是0,2就表示了写权限,因为原来的
  O_WRONLY是1.而刚好FMODE_READ是1,FMODE_WRITE是2.

- inode.i_mode是一个包含文件类型和访问权限的成员.
- 如果文件类型是一个目录且不会对它写的话,就可以通过
  #+BEGIN_EXAMPLE
...
- 接下来是检查文件类型和写权限了,
  ...
  若是fifo或socket,那么要把O_TRUNC给去掉.注意参数acc_mode是被通过flag转的,在open_namei()里
  转.
  #+BEGIN_EXAMPLE
  ...
  若是设备文件,要先检查有没有MNT_DODEV标志再清O_TRUNC
  #+BEGIN_EXAMPLE
...
#define S_PRIVATE	512	/* Inode is fs-internal */
  #+END_EXAMPLE
- 对于一个只能追加的文件,若不能只设置写访问,还要加多一个O_APPEND标志才行.
...
- O_NOATIME: Do not update the inode's last access time
  ...
  #+END_EXAMPLE
- 调用break_lease()
- 这个写的访问还真多要检查的.到了这里之后还要检查VM_DENYWRITE.这个检查是调用
  get_write_access()
- 为什么只有设置了O_TRUNC才会调用locks_verify_locked()来检查它的强制锁呢?
  #+BEGIN_EXAMPLE
	if (flag & O_TRUNC) {
  ...
  因为强制锁这些记录锁可以对文件的某一部分加锁,但是因为以O_TRUNC打开文件会把整个文件截为0,
  所以无论强制锁加在哪个部分都会影响到.
- 若设置了O_TRUNC,那么就调用do_truncate()把文件截成长度为0.
  #+BEGIN_EXAMPLE
			error = do_truncate(dentry, 0);
  #+END_EXAMPLE
- 所以这个函数主要是检查文件能不能按照要求来打开,还截短了.
** int permission(struct inode *inode, int mask, struct nameidata *nd)
*** fs/namei.c:
- 这个函数由may_open()调用
- 在open_namei()已经把参数flag通过下面的宏转成chmod的形式了,之后再传给may_open(),转而再传
  给这个函数.
  #+BEGIN_EXAMPLE
#define ACC_MODE(x) ("\000\004\002\006"[(x)&O_ACCMODE])
  #+END_EXAMPLE 
...
- 接下来查看写访问是否被允许
  ...
  如果文件只被读,且这个文件是 _普通文件_ 或是 _目录_ 或是 _符号链接_,就不能写访问.
  
  除了上面3种文件类型之外,还有chr,blk,fifo,sock
  #+BEGIN_EXAMPLE
...
#define S_ISSOCK(m)	(((m) & S_IFMT) == S_IFSOCK)
  ...
  为什么只要检查上面的3种呢?应该是因为只有写这3种文件的时候才会操件文件所在的文件系统的磁
  盘.而其它的不会对磁盘操作.
...
#define IS_IMMUTABLE(inode)	((inode)->i_flags & S_IMMUTABLE)
  #+END_EXAMPLE 
  inode.i_flags:Filesystem mount flags
- 要先清掉MAY_APPEND,这个标志是在open_namei()里根据O_APPEND得到的
  #+BEGIN_EXAMPLE
	/* Ordinary permission routines do not understand MAY_APPEND. */
	submask = mask & ~MAY_APPEND;
  #+END_EXAMPLE
- 若inode.i_op.permission()为空,就调用generic_permission()
** int generic_permission(struct inode *inode, int mask, int (*check_acl)(struct inode *inode, int mask))
*** fs/namei.c:
- FSUID, FSGID -- 用于文件系统访问检查的用户ID和组ID；一般等于相应的EUID和EGID；这是一个
  Linux特有的属性。

  Linux特有的进程属性FSUID是用来允许NFS服务器一类的程序把自己的文件系统权限限制在某些给定的
  UID上，而不给这些UID向进程发送信号的许可。一旦EUID被改变，FSUID就被改为新的EUID值；FSUID
  的值可以用Linux独有的调用setfsuid(2)单独进行设置。注意，非 root调用者只能把FSUID设置为当
  前的RUID、EUID、SEUID或当前的FSUID。
...
*** fs/namei.c:
- 这个函数主要是操作inode.i_writecount,它的值与MAP_DENYWRITE/VM_DENYWRITE有关
...
*** fs/attr.c:
- 这个函数好像是通知某个dentry的文件属性改变了,改变了什么和怎么妀变都在第二个参数有说明.
- iattr.ia_valid应该就是说明改变了什么,其它的成员应该就是说明怎么改变的.ia_valid的标志有
  #+BEGIN_EXAMPLE
...
#define ATTR_KILL_SGID	4096
  #+END_EXAMPLE
- 调用current_fs_time()计算时间,这个函数好像做一个时间的转换
** int do_truncate(struct dentry *dentry, loff_t length)
*** fs/open.c:
- 这个函数是用来截短文件的
- 其实就是调用notify_change()来修改文件的属性,有长度和CTIME属性.
** struct file *get_empty_filp(void)
*** fs/file_table.c:
- 这个函数用来获得一个struct file
- 首先判断有没有超过数量限制
  #+BEGIN_EXAMPLE
	if (files_stat.nr_files < files_stat.max_files ||
				capable(CAP_SYS_ADMIN)) {
  #+END_EXAMPLE
- 调用kmem_cache_alloc()分配struct file
- 这里设置了file.f_uid/f_gid为current.fsuid/fsgid.  原来fsuid/fsgid是这样被用的.
** struct file *dentry_open(struct dentry *dentry, struct vfsmount *mnt, int flags)
*** fs/open.c:
- 这个函数被调用来打开dentry指定的文件.所谓的打开主要是建一个合适的struct file.
- file.f_flags就是在filp_open()转换过的flags
- file.f_mode就是除了有读/写外,还加上FMODE_LSEEK,FMODE_PREAD,FMODE_PWRITE
  ...
  f_flags指的是以什么方式打开,而f_mode指的是打开之后能做什么.
- f_mapping是用inode.i_mapping的.i_mapping:Pointer to an address_space object
- f_dentry是用参数的
- f.vfsmnt是用参数的
- f.f_pos为0
- f.f_op是用inode.i_fop
- 把这个file插入到inode.i_sb.s_files链表里
- 到这里还要调用file.f_op.open()方法,这就是驱动自已定义的方法.
- file.f_flags:Flags specified when opening the file
  
  为什么要清O_CREAT,O_EXCL,O_NOCTTY,O_TRUNC呢?
  #+BEGIN_EXAMPLE
	f->f_flags &= ~(O_CREAT | O_EXCL | O_NOCTTY | O_TRUNC);
  #+END_EXAMPLE 
- 初始化预读窗口,为什么要这样呢?为什么读写都要呢?
- 若设置了O_DIRCT,那么就要检查file.f_mapping.a_ops.direct_IO方法是否被定义.
** asmlinkage ssize_t sys_read(unsigned int fd, char __user * buf, size_t count)
*** fs/read_write.c:
- 先调用fget_light()来获取fd对应的file object.
- 再调用file_pos_read()来获取文件指针,其实是就是file.f_pos.
- 
...
- 先要看是不是读访问的
  #+BEGIN_EXAMPLE
	if (!(file->f_mode & FMODE_READ))
		return -EBADF;
  #+END_EXAMPLE
- 再看有没有定义file.f_op.read/aio_read
  #+BEGIN_EXAMPLE
...
- 接下来看看用户空间可以访问不
  #+BEGIN_EXAMPLE
	if (!file->f_op || (!file->f_op->read && !file->f_op->aio_read))
		return -EINVAL;
  #+END_EXAMPLE
- 调用rw_verify_area()作一些区域性的检查,一次读/写的字节数是否过大,还有强制锁的事.
- 接下来就是比较核心的部分了,调用自定义的read(),若没有就调用do_sync_read(),do_sync_read()
  最后就是调用自定义的aio_read().注意是先调用read()的.
- 调用完自定义的读函数之后就调用dnotify_parent()
** int rw_verify_area(int read_write, struct file *file, loff_t *ppos, size_t count)
*** fs/read_write.c:
- file.f_maxcount: Maximum number of bytes that can be read or written with a single
  operation

  首先判断要读取的字节数有没有超过file_f_maxcount

- 接下来看文件指针,自身和加上count都不能为负.

- 接下来就是处理强制锁了,
  
  MS_MANDLOCK: Mandatory locking allowed.这个标志是执行mount命令时指定的,后来传给
  inode.i_mode的
  ...
  MANDATORY_LOCK()这个宏就是作上面这两种检查,如果上面的这两种检查都存在,那么就要在指定的区
  域加锁了.要调用lock_mandatory_area()

  
** struct file fastcall *fget_light(unsigned int fd, int *fput_needed)
*** fs/file_table.c:
- task_struct.fs的类型是files_struct. files_struct.count:Number of processes sharing this
  table.
- 关于竞争注释有说明
  #+BEGIN_EXAMPLE
 ...
 * There must not be a cloning between an fget_light/fput_light pair.
  #+END_EXAMPLE
- 若文件描述表没有被共享,那么直接调用fcheck_files(),这个函数就是直接返回files.fd[fd].
- 若文件描述表被共享,那么要加files.file_lock锁,还有一点很不一样的是:还要增加找的file的计数
  器.
  
  这样说明,这个函数的操作有点大胆
** asmlinkage long sys_close(unsigned int fd)
*** fs/open.c:
- 在files.fd里找出合适的struct file
- 要把fd对应的在close_on_exec的位给清掉
- 因为一个fd不用了,所以调用__put_unused_fd()来处理files.open_fds和files.next_fd.
- 调用filp_close()
** int filp_close(struct file *filp, fl_owner_t id)
*** fs/open.c:
- 这个函数的返回值有两种
  
  一个是使用file.f_error, 接着file.f_error清零
  
  若file.f_error没有错误码,那么就调file.f_op.flush()返回的错误码,若有,就不会使用flush()返
  回的.

- 这个函数主要调调用file.f_op_flush()后调用dnotify_flush()

- 调用locks_remove_posix()处理锁的事

- 调用fput()释放file object.
** void dnotify_flush(struct file *filp, fl_owner_t id)
*** fs/dnotify.c:
- 若传入的filp不是一个目录的file object ,那么就返回.
  #+BEGIN_EXAMPLE
	if (!S_ISDIR(inode->i_mode))
		return;
  #+END_EXAMPLE 
  
  所以可以看出这个函数只处理目录的file object

- 注意函数使用的inode是从filp里获得的,所以它们是对应的.
** static void redo_inode_mask(struct inode *inode)
*** fs/dnotify.c:
- 这个函数就是把inode.i_dnotify链表里所有的dnotify_struct.dn_mask全都 _或_ 起来,最后把
  _DN_MULTISHOT_ 给清掉, 再给inode.i_dnotify_mask
- 在dnotify_flush()里这个函数被调用一次就退出循环了.
** not function
*** 12.7
- It is possible to lock an arbitrary region of a file (even a single byte) or to lock the
  whole file (including data appended in the future). Because a process can choose to lock
  only a part of a file, it can also hold multiple locks on different parts of the file.
- 若删除文件中间的一部分内容,那么文件在磁盘的存放位置是如何变化的?
- POSIX标准的锁是用fcntl()实现,这个锁是建议锁,可以锁 _部分_ 或 _全部_.
  
  ulk:The POSIX standard requires a file-locking mechanism based on the fcntl( ) system
  call. It is possible to lock an arbitrary region of a file (even a single byte) or to
  lock the whole file (including data appended in the future). Because a process can
  choose to lock only a part of a file, it can also hold multiple locks on different parts
  of the file.
  
  This kind of lock does not keep out another process that is ignorant of locking. Like a
  semaphore used to protect a critical region in code, the lock is considered "advisory"
  because it doesn't work unless other processes cooperate in checking the existence of a
  lock before accessing the file. Therefore, POSIX's locks are known as advisory locks .
- 传统的BSD用flock()实现 _建议锁_,但只能锁 _全部_.
- 传统的system v提供fcntl()的一个接口lockf()
- system v3引入了 _强制锁_, 在调用 _open()_, _read()_, _write()_ 时会检查,不能冲突.
- linux支持 _建议锁_, _强制锁_.支持 _fcntl()_ (POSIX)(系统调用), _flock()_( BSD)(系统调用),
  _lockf()_ (system v)(函数库) .
- 在同一个区域只能同时出现多个读锁,不能同时出现读和写锁,和多个写锁.
- 在linux,flock()是可以实现强制锁的
- linux 在fcntl()引入的强制锁就 _lease_,若一个进程要访问被lease的区间,那么会被 _block_,拥
  有lease的进程会被 _informed_,那么它就要 _update_ 文件再 _release_ lease,若超过一定的时间
  还没有relase, _kernel_ 会自动release.
- 要实现 _建议锁_ 可以使用 _flock()_ 和 _fcntl()_.
- _flock()_ 和 _fcntl()_ 之间的锁互不干扰,为什么要这样做呢?ulk:This has been done on
  purpose in order to avoid the deadlocks occurring when an application using a type of
  lock relies on a library that uses the other type.
- 要想使用 _强制锁_ ,有3步:
  
  mount命令的 _MS_MANDLOCK_ 
  
  设置SGID,清组执行位
  
  调用fcntl()

- read(),write(),以O_TRUNC的open()会处理锁的事.

- 作用于同一文件的 _lock_file_ 文件锁结构体都是放在inode.i_flock链表里.

- lock_file.fl_wait:When a process issues a blocking system call to require an exclusive
  lock while there are shared locks on the same file, the lock request cannot be satisfied
  immediately and the process must be suspended. The process is thus inserted into a wait
  queue pointed to by the fl_wait field of the blocked lock's file_lock structure.
- _file_lock_list_, _blocked_list_, _lock_file.fl_link_ :有一些 lock_file结构体所描述的锁已经
  成功获取,但一些还没有,为了分清这两种情况,所以就有file_lock_list和blocked_list这两个链表
  了,相应的lock_file.fl_link就插到其中的一个链表里去
- FL_FLOCK锁指的是建议锁
- http://www.ibm.com/developerworks/cn/linux/l-cn-filelock/index.html
  共享模式锁

  Linux 中还引入了两种特殊的文件锁：共享模式强制锁和租借锁。这两种文件锁可以被看成是强制锁的
  两种变种形式。共享模式强制锁可以用于某些私有网络文件系统，如果某个文件被加上了共享模式强
  制锁，那么其他进程打开该文件的时候不能与该文件的共享模式强制锁所设置的访问模式相冲突。但
  是由于可移植性不好，因此并不建议使用这种锁。
- 租借锁

  采用强制锁之后，如果一个进程对某个文件拥有写锁，只要它不释放这个锁，就会导致访问该文件的
  其他进程全部被阻塞或不断失败重试；即使该进程只拥有读锁，也会造成后续更新该文件的进程的阻
  塞。为了解决这个问题，Linux 中采用了一种新型的租借锁。

  当进程尝试打开一个被租借锁保护的文件时，该进程会被阻塞，同时，在一定时间内拥有该文件租借
  锁的进程会收到一个信号。收到信号之后，拥有该文件租借锁的进程会首先更新文件，从而保证了文
  件内容的一致性，接着，该进程释放这个租借锁。如果拥有租借锁的进程在一定的时间间隔内没有完
  成工作，内核就会自动删除这个租借锁或者将该锁进行降级，从而允许被阻塞的进程继续工作。
- http://blog.csdn.net/yebanghua/article/details/7301904
  
  概念:文件锁,记录锁,读写锁,共享锁,互斥锁
  
  读写锁是在记录锁里分类的
  
  Unix系统对文件加锁有两种粒度:文件锁和记录锁，
  
  文件锁用来锁定整个文件，
  
  而记录锁可以锁定文件的部分区域甚至一个字节，进程通过为文件设置多个记录锁，可以实现文件中
  不同区域的数据的读写同步，因此记录锁最为常见。
  
  记录锁根据访问方式的不同，又分为读锁和写锁。
  
  读锁允许多个进程同时进行读操作，也称共享锁。
  
  文件加了读锁就不能再设置写锁，但仍允许其他进程在同一区域再设置读锁。
  
  写锁的主要目的是隔离文件使所写内容不被其他进程的读写干扰，以保证数据的完整性。
  
  写锁一旦加上，只有上锁的人可以操作，其他进程无论读还是写只有等待写锁释放后才能执行，故写
  锁又称互斥锁，写锁与任何锁都必须互斥使用。
- 文件锁和记录锁是按照锁的 _粒度_ 分类的.按 _实现机制_ 分类有建议锁和强制锁.
  
  建议锁开销小，可移植性好，符合POSIX标准的文件锁实现，在数据库系统中应用广泛，特别是当多个
  进程交叉读写文件的不同部分时，建议锁有更好的并行性和实时性。
  
  这说明POSIX的锁的建议锁.
  
  flock和posix lock都共享一组机制，系统调用fcntl() 符合 POSIX 标准的文件锁实现，功能比
  flock强大，可以支持记录锁。flock() 系统调用是从 BSD 中衍生出来的，只能支持整个文件的锁。

- http://zohead.com/archives/linux-fcntl-advisory-mandatory-lock/
  
  通过这文章描述来看,建议锁和强制锁是不能对同一个文件使用的,若挂载时没用-o且没有去掉程序的
  组执行权限和增加程序的设置组ID权限时,执行文章的代码就是使用建议锁,否则就是强制锁.所以在
  fcntl()函数的参数里没有指定是否使用强制锁,只是在函数内部通过判断挂载标志和文件属性来确定
  使用的是什么锁.
  
  使用 BSD 的 flock 函数时不能使用强制锁，见 Linux kernel 源码下 mandatory-locking.txt 中的
  说明：Mandatory locks can only be applied via the fcntl()/lockf() locking interface – in
  other words the System V/POSIX interface. BSD style locks using flock() never result in
  a mandatory lock.
  
  那么租借锁又是如何与建议锁/强制锁协调的呢?租借锁使用F_SETLEASE,F_GETLEASE,建议锁/强制锁
  使用F_SETLK/F_GETLK/F_SETLKW

- fcntl()好像没有分开建议锁和强制锁,
  
  http://bbs.chinaunix.net/thread-1499379-1-1.html
  
  系统默认fcntl都是建议性锁，强制性锁是非POSIX标准的。如果要使用强制性锁，要使整个系统可以
  使用强制性锁，那么得需要重新挂载文件系统，mount使用参数 -0 mand打开强制性锁，或者关闭已加
  锁文件的组执行权限并且打开该文件的set-GID权限位。
 ...
 *	processes read and write access respectively.
  #+END_EXAMPLE 
- 若想给文件加建议锁,但是file object不能读和写,那么就返回错
  #+BEGIN_EXAMPLE
	if (!unlock && !(cmd & LOCK_MAND) && !(filp->f_mode & 3))
		goto out_putf;
  #+END_EXAMPLE 
- 若有LOCK_NB标志,那么就设置file_lock.fl_flags的FL_SLEEP
- 接下来若自定义了file.f_op.flock()那么就执行,否则就执行flock_lock_file_wait()(这是大多数
  的情况)
** static int flock_make_lock(struct file *filp, struct file_lock **lock,
*** fs/locks.c:
- 这个函数创建一个struct file_lock和初始化它
- 调用flock_translate_cmd()把cmd转成另一种形式赋给file_lock.fl_type,结果有几种
...
#define LOCK_RW		192	/* ... Which allows concurrent read & write ops */
  ...
  基于fcntl()的 _强制锁_ 才就lease
- 奇怪的一点是为什么flock_translate_cmd()把强制锁转成bsd的flock()的,而建议锁转成posix的
  fcntl()和lockf()形式的.还是说LOCK_MAND,LOCK_READ,LOCK_WRITE,LOCK_RW不只是flock()的,而是
  所有使用强制锁的函数共用的.
- 这里使用的标志是FL_FLOCK,不管是不是强制锁
  #+BEGIN_EXAMPLE
...
#define FL_SLEEP	128	/* A blocking lock */
  #+END_EXAMPLE 
- 根据flock_translate_cmd(),锁的类型有(lock_file.fl_type)
  #+BEGIN_EXAMPLE
...
#define LOCK_RW		192	/* ... Which allows concurrent read & write ops */
  #+END_EXAMPLE 
- 这个函数无论是建议锁还是强制锁,都设置fl_end为OFFSET_MAX,也没有设置fl_start
** int flock_lock_file_wait(struct file *filp, struct file_lock *fl)
*** fs/locks.c:
- 
** static int flock_lock_file(struct file *filp, struct file_lock *new_fl)
*** fs/locks.c:
- 加一个锁要获得大内核锁,不管是不是强制锁.
- 为什么在第一次扫描过程中,发现链表里有POSIX锁就退出呢?好像可以保证链表里若有POSIX锁就没有
  FL_FLOCK锁似的
  
  为什么有LEASE锁时是continue呢?
- 要注意第一个循环的目的应该是调用locks_delete_lock()来删除
- ulk:If an FL_FLOCK lock for the same file object has been foundthus the process is
  changing an already owned read lock into a write one (or vice versa)gives some other
  higher-priority process, in particular every process previously blocked on the old file
  lock, a chance to run by invoking cond_resched( ).
  
  第一个循环会在什么情况下有以上所说的情况呢?就是在IS_POSIX()为假,IS_LEASE()为假,filp等于
  fl.fl_file, new_fl.fl_type不等于fl.fl_type,就是想妀变一个file object的FL_FLOCK类型.这时
  found会为1,同执行完locks_delete_lock()后cond_resched()
- 为什么在第一个循环之后判断new_fl.fl_type为F_UNLCK就会返回0呢?
  #+BEGIN_EXAMPLE
  ...
  ulk:If the process is performing an unlock (LOCK_UN), nothing else needs to be done: the
  lock was nonexisting or it has already been released, thus returns 0.
  
  所以执行到这里之后只两种情况,就是原来的锁本不在,或已被释放.那么第一个循环只有三种情况会
  退出循环执行到这里,第一种就是循环到最后一结点而结束,另一种就是IS_POSIX()为真,最后一种就
  是执行完locks_delete_lock().所以前两种情况就是锁本不在,后一种情况就是锁被释放掉.
  
  若是被释放掉的,那么就要调用cond_resched().
- 再看ulk的这段话,ulk:Searches the list that filp->f_dentry->d_inode->i_flock points
  to. If an FL_FLOCK lock for the same file object is found, checks its type (LOCK_SH or
  LOCK_EX): if it is equal to the type of the new lock, returns 0 (nothing has to be
  done). Otherwise, the function removes the old element from the list of locks on the
  inode and the global file lock list, wakes up all processes sleeping in the wait queues
  of the locks in the fl_block list, and frees the file_lock structure.
  
  这段话又对应什么的代码呢?
  
  对于ulk说的返回0情况,执行过程是filp等于fl.fl_file, new_fl.fl_type也等于fl.fl_type,这时就
  跳到out标签去执行.
  
  至于ulk说的否则后的话,就是locks_delete_lock()执行的任务:从inode的文件锁链表里删除该锁,同
  是也从global file lock list这个激活的锁链表里删除该锁,最后唤醒休眠在fl_block里的进程.
  
  奇怪的是在什么时候会把进程插入到fl_block里呢?

- 执行到第二个循环主要是想把锁插入到锁链表, ulk:Searches the list of locks on the inode
  again to verify that no existing FL_FLOCK lock conflicts with the requested one. There
  must be no FL_FLOCK write lock in the list, and moreover, there must be no FL_FLOCK lock
  at all if the process is requesting a write lock.
 
  flock_locks_conflict()返回0就表示没有冲突,若发现有冲突的话,就会往下执行,往下执行的结果是
  先看这个锁允不允许休眠,接着就退出.

- 结合两个循环来看POSIX锁时,就会发现若是POSIX锁,就会调用locks_insert_lock().
- 进程是在调用locks_insert_block()时放到锁的等待队列的
- 若不是插在等待队列,就是插在file_lock_list这个激活的链表里,这个要调用locks_insert_lock(),
  这个函数还把锁插入到了inode.i_flock.
** static void locks_delete_lock(struct file_lock **thisfl_p)
*** fs/locks.c:
- 奇怪的是,这个函数好像有一个错误,就是*thisfl_p.fl_next可能为NULL,那么为什么可以赋给
  *thisfl_p呢?可以啊,结果就是*thisfl_p就是NULL而已,没什么大不了的.
** static int flock_locks_conflict(struct file_lock *caller_fl, struct file_lock *sys_fl)
*** fs/locks.c:
- 判断FL_FLOCK锁有没有冲突
- 用在同一个file object的FLOCK锁是不会冲突的
  #+BEGIN_EXAMPLE
...
- 第一个判断
  ...
  这个函数主要是检查FL_FLOCK的,所以不是FL_FLOCK锁,就不处理了,但总要一个返回结果,所以就返回
  了不冲突(0).
  
  若是FL_FLOCK锁,且是相同的file object,也是不冲突的.为什么呢?难道是因为同一个file object是
  被共享的,所以让这些共享进程自已去协调吗?而不管是不是强制锁.
- 若执行到了第二个判断,就是说明sys_fl的锁是FL_FLOCK锁且在是同一个file object.
- 第二个判断是不处理旧或新的强制锁,若有强制锁就返回不冲突
- 接下来就是调用locks_conflict()了,这个函数处理过程就要ulk所说的那样:There must be no
  FL_FLOCK write lock in the list, and moreover, there must be no FL_FLOCK lock at all if
  the process is requesting a write lock.
** not function
*** 12.7.4
- 一个FL_POSIX锁是和一个进程还有一个inode相关连的,不能缺一.这点可能与FL_FLOCK不一样
- FL_POSIX锁不会因调用fork()而产生继承.这点可能也与FL_FLOCK不一样
- www.ibm.com/developerworks/cn/linux/l-cn-filelock/index.html
  
  F_GETLK：进程可以通过它来获取通过 fd 打开的那个文件的加锁信息。执行该操作时，lock 指向的
  结构中就保存了希望对文件加的锁（或者说要查询的锁）。如果确实存在这样一把锁，它阻止 lock
  指向的 flock 结构所给出的锁描述符，则把现存的锁的信息写到 lock 指向的 flock 结构中，并将
  该锁拥有者的 PID 写入 l_pid 字段中，然后返回；否则，就将 lock 指向的 flock 结构中的
  l_type 设置为 F_UNLCK，并保持 flock 结构中其他信息不变返回，而不会对该文件真正加锁。
  ...
  与租借锁相关的 cmd 参数的取值有两种：F_SETLEASE 和 F_GETLEASE。其含义如下所示：
    ...
    F_SETLEASE：根据下面所描述的 arg 参数指定的值来建立或者删除租约：
        ...
        F_RDLCK：设置读租约。当文件被另一个进程以写的方式打开时，拥有该租约的当前进程会收
  到通知
        
        F_WRLCK：设置写租约。当文件被另一个进程以读或者写的方式打开时，拥有该租约的当前进
  程会收到通知
        
        F_UNLCK：删除以前建立的租约
        
    F_GETLEASE：表明调用进程拥有文件上哪种类型的锁，这需要通过返回值来确定，返回值有三种：
  F_RDLCK、F_WRLCK和F_UNLCK，分别表明调用进程对文件拥有读租借、写租借或者根本没有租借
...
*** fs/locks.c:
- 调用这个函数主要是处理强制锁的事
- 在do_fcntl()的cmd为F_SETLK或F_SETLKW时才调用的.
- 若文件有一个共享内存映射,那么就不能加制锁.
  #+BEGIN_EXAMPLE
  ...
  #+END_EXAMPLE 
- 接下调用flock_to_posix_lock()把struct flock转成struct file_lock
- 要检查F_RDLCK对应FMODE_READ,F_WRLCK对应FMODE_WRITE
- 调用file.f_op.lock()
- 接下来是调用__posix_lock_file()
** static int flock_to_posix_lock(struct file *filp, struct file_lock *fl, struct flock *l)
*** fs/locks.c:
- 这个函数主要是用用户态传入的struct flock结构体里的数据初始化struct file_lock里的东西.
- 文件的被锁的区域是如何确定的呢? 使用3个东西,一个是SEEK_SET,SEEK_CUR,SEEK_END,第二个是被
  锁区域的起始位置相对于SEEK_SET或SEEK_CUR或SEEK_END的偏移量,最后一个是被锁区域的长度
- file_lock.fl_flags设成了FL_POSIX
...
- 扫描inode里所有的锁.
  ...
  接下来是检查冲突的问题,使用posix_locks_conflict().
  
  若是同一个拥有者就不冲突,file_lock.fl_owner
  
  若区间不重合就不冲突,这点FL_FLOCK没有检查
  ...
  #+BEGIN_EXAMPLE
			if (!posix_locks_conflict(request, fl))
				continue;
  #+END_EXAMPLE 
  所以往下执行的都是有冲突的POSIX锁,若不能休眠(FL_SLEEP),那么就退出
  #+BEGIN_EXAMPLE
			if (!(request->fl_flags & FL_SLEEP))
				goto out;
  #+END_EXAMPLE 
  
  不明白为什么会出现死锁呢?判断是否死锁的方法就是看blocked_list里是否有同样的拥有者,但是
  blocked_list里的进程不是都已经休眠了吗?
  
  在posix_locks_deadlock()里会调用posix_same_owner()来检查是否有同一个拥有者,之前在
  posix_locks_conflict()同样也会调用,但是能执行到这里并不能确定posix_locks_conflict()调用
  的posix_same_owner()是什么情况.
  
  posix_locks_deadlock()是这样检查死锁的,先用被加入的锁(caller_fl)与在__posix_lock_file()
  里当前处理的锁比较是不是有相同的拥有者,这两个锁是同一个inode的,若相同,那么就表示有死锁,
  所以可以看出inode.i_flock里的所有锁都不可能有相同的拥有者.比较完这个后就是用被加入的锁
  (caller_fl)与block_list链表里的被阻塞的锁比较,若有相同的也认为有冲突.
  
  关于file_lock.fl_block:若有锁A被锁B阻塞,B是激活锁,那么A的file_lock.fl_block就插到B的
  file_lock.fl_block里?

  若即可以休眠又没有死锁,那么就可以调用locks_insert_block()把锁插入到file_lock.fl_block里,
  若想插入到blocked_list链表里,就必须是POSIX锁.
  #+BEGIN_EXAMPLE
	if (IS_POSIX(blocker))
		list_add(&waiter->fl_link, &blocked_list);
  #+END_EXAMPLE

- 若能正常地退出第一个循环,而不是跳到out标签处,那么就说明被加的锁是一个没有与其它在
  inode.i_flock的锁有冲突的.
- FL_ACCESS只是看锁会不会与其它的锁冲突,从代码来看若没有冲突就会返回0,若冲突就会返回其它的
  错误码
- 以下的代码是找到在inode.i_flock链表里第一个是POSIX锁且有相同拥有者的锁放到befor变量里.
  #+BEGIN_EXAMPLE
  ...
  #+END_EXAMPLE
- while()循环是处理与被加入锁有相同拥有者的锁.有点奇怪的是好像有相同拥有者的POSIX锁是连续
  的似的.
  
  1. 若是相同类型的锁
     
     若新加入的锁的区间在旧锁的后面就再处理下一个旧锁
     
     若新锁区间在旧锁的前面,那么就跳出循环,为什么不处理下一个锁了呢?
     
     若以上的两种情况都不是,那么就说明有重合,所以就把它们合并起来
     #+BEGIN_EXAMPLE
...
** not function
*** 13.1
- ulk:While accessing I/O ports is simple, detecting which I/O ports have been assigned to
  ...
  could occur. To prevent such situations, the kernel keeps track of I/O ports assigned to
...
- A resource represents a portion of some entity that can be exclusively assigned to a
  ...
  fields are shown in Table 13-1. All resources of the same kind are inserted in a
  tree-like data structure; for instance, all resources representing I/O port address
  ranges are included in a tree rooted at the node ioport_resource.
- 父的资源范围包含子的资源范围.资源树的结构就是这样的,由parent,child,sibling
- http://www.cnblogs.com/image-eye/archive/2012/01/29/2330838.html
  
  kset.kobj指针所指向的kobject的kset好像没有指向什么对吗?还是说指向包含这个kset的kset的呢?
  应该是这样的.
  
  kset里的内嵌的kobject的件用是:一个kobject加入一个kset，主要是kobject结构体中的相关字段记
  录了对应的kset信息
- http://www.360doc.com/content/11/1110/23/1317564_163453770.shtml
  
  一个kobject对自身关不感兴趣，它存在的意义在于把高级对象连接到设备模型上。</STRONG></EM>高
  级对象包括：总线，设备，设备驱动等。要把它们抽象起来，组织成一个模型，把必须要有一个中间
  物，这个中间物就是kobject。为此，kobject总是被嵌入到这些高级对象中，很少有一个单独的
  kobject对象。理解这个对于理解设备模型很重要.
- struct cdev里嵌入了struct kobject有什么作用呢?
- ldd3:一旦设置了kset并把它添加到系统中,将在sysfs中创建一个目录.kobject不必在sysfs中表示,
  但是kset中的每一个kobject成员都将在sysfs中得到表示.
- ldd3:kset在一个标准的内核链表中保存了它的子节点.在大多数情况下,所包含的kobject会在它们的
  parent成员中保存kset(严格地说是其内嵌的kobject)的指针.
  
  保存到kset链表里的节点的kobject实际上是被嵌入到其他类型中(就如struct cdev里的kobject一
  样).甚至可能是其他的kset.
  
  ldd3:一个kobject的父节点不一定是包含它的kset.(这种情况非常少)
  
  只有设置了kset才会在sysfs里创建目录,而有kobject的其它类型则不会在sysfs里创建目录.
  
  ldd3:kset中也有一个指针(在ktype成员中)指向kobj_type结构,用来描述它所包含的kobjcet.该类型
  的使用的优先于kobject中的ktype.因此在典型应用中,kobject中的ktype成员被设置为NULL.因为
  kset中的ktype成员是实际上被使用的成员.
  
  所以kset.ktype是用来描述在kset那个kobject链表里的kobject的.且是优先使用.
  
  ldd3:子系统通常(但不一定)显示在sysfs分层结构中的顶层.
  
  ldd3:每一个kset都必须属于一个子系统.
  
  可以通过kset.subsystem找到包含该kset的子系统,但是不能从subsystem里找到包含的多个kset.
  
  ldd3:对于sysfs中的每个目录,内核中都会存在一个对应的kobject.每一个kobject都输出一个或者多
  个属性,它们在kobject的sysfs目录中表现为文件,其中的内容由内核生成.这部分内容揭示了kobject
  和sysfs在底层是如何交互的.
  
  所以sysfs的文件与kobject不是一对一关系,只有目录与kobject才是一对一的关系,里面的文件是因
  为kobject而输出的.文件是由内核生成的,不用特意去生成.
  
  ldd3:只要调用kobject_add(),就能在sysfs中显示kobject.
  
  ldd3:kobject在sysfs中的入口始终是一个目录,因此,对kobject_add的调用将在sysfs中创建一个目
  录.通常这个目录包含一个或多个属性.
  
  所以一个目录一定有一个kobject对应的.
  
  ldd3:sysfs入口在目录中的位置对应于kobject的parent指针.如果调用lobject_add的时候,parent是
  NULL,它将被设置为嵌入到新kobjcect的kset中的kobject,这样,sysfs分层结构通常与kset创建的内
  部结构相匹配.如果parent和kset都是NULL,则会在最高层创弪sysfs目录,在这通常不是我们所期望的.
  
  那么parent指向其它的kset内嵌的kobject和指向把自已给内嵌的kset的目录布局又是怎么样的呢?
  
  现在有一个疑问:就是kobject.kset一定是指向包含该kobject的kset吗?应该是的,假设有A,B,C三个
  kset,当然都有内嵌的kobject,现在把它们连接起来,假如A是父目录,如果没有kobjcet.kset表示的话
  B和C是不知道其它两个kset那个是父目录的.那parent又有什么用呢?真正的目录视图到底是
  kobject.kset还是kobject.parent在起作用呢?可能不是kset,有kobject.kset是为了使用kset里的
  kobj_type?不会使用kobject.kset.kobj_type的,最多只会使用kobject被内嵌的kset的ktpe.有答案
  了:之前有说过,ldd3:sysfs入口在目录中的位置对应于kobject的parent指针.ldd3:一个kobject的父
  节点不一定是包含它的kset(这样的结构非常少见).
  
  若有kset A和B,它们都有内嵌的kobject,若B.kobj.parent指向A.kobj,这时目录和目录里的文件应该
  是这样生成的:A生成一个相应的目录C,这个目录里有一个目录D对应B,目录C里生成的一些文件是根据
  A.ktype.default_attrs这个sturct attribute链表生成的,若A.ktype为空,那么目录C里生成的一些
  文件就是根据A.kobj.ktype.default_attrs.要生成多个文件,struct attribute链表就要有多个节点.
  
  kobj_type.sysfs_ops是读写kobj_type.default_attrs里的方法.
- 关于二进制属性是如何使用的呢?它的类型是struct bin_attribute,所以不能像非默认属性那样可以
  插在kobject.ktpye.attr链表里.二进制属性是用来上传固件用的.struct bin_attribute里有一个
  struct attribute类型的成员,而这个attribute应该是被链接到kset.ktype.default_attrs里的
- kobject的添加和删除都会引起热插拔事件,事件会从内核态发到用户态,从而引起/sbin/hotplug的调
  用,hotplug会通过加载驱动程序,创那设备节点,挂载分区或者其他的动作来响应事件.
  
  调用kobject_add或kobject_del时才会真正产生这些事件.
  
  热插拔的操作函数在kset.hotplug_ops里,这可能也是为什么kobject有一个kset的原因吧.
  
  ldd3:如果在kset中不包含一个指定的kobject,内核将在分层结构中进行搜索(通过parent指针),直到
  找到一个包含有kset的kobject为止,然后使用这个kset的热插拔操作.
- ldd3:无论什么时候，当内核要为指定的kobject产生事件时，都要调用kset_hotplug_ops.filter函
  数.如果filter返回0,将不产生事件.因此该函数给kset代码一个机会,用于决定是否向用户空间传递
  特定的事件.
  
  所以可以filter函数来确定是否逡中用户通知事件
- ldd3:任何热插拔脚本所需要知道的信息将通过环境变量传递.最后一个hotplug方法(hotplug)会在调
  用脚本前,提供添加环境变量的机会.ldd3有关于这个函数的参数的描述.
- 总线的结构体是struct bus_type.ldd3:每个总线都有自已的子系统.然而这些子系统并不在sysfs中
  的顶层,相反钗,我们会在总线子系统下面发现它们.一个总线包含两个kset,分别代表了总线的驱动程
  序和插入总纯的所有设备
  
  所以总线不在最顶层,一个kset指总线驱动程序,一个kset指 _所有_ 设备.
- 总线的结构体是struct bus_type.
- 总线的match方法:ldd3:当一个总线上的新设备或者新驱动程序被添加时,会一次或多次调用这个函数.如
  果指定的驱动程序能够处理事务定的设备,该函数返回非零值.必须在总线层上使用该函数,因为那里
  存在着正确的逻辑.核心内核不知道如何为每个总线类型匹配设备和驱动程序.
  
  match方法原来是 _总线_ 的.
- 总线的hotplug方去:ldd3:在为用户空间产生热插拔事件前,这个方法允许总线添加环境变量.其 _参
  数_ 与 _kset_ 的hotplug方法相同.
  
  那么这两个方法调用有什么不同吗?总线的hotplug是给总线添加环境变量的,kset的hotplug是给kset
  添加环境变量.
- bus_type里有一个struct bus_attribute的属性结构体表示总线的属性.
- struct bus_type里有kset没有kobject,但是struct device里 _没有_ kset _有_ kobject,所以
  struct device不会做为一个容器来包含其它的kobject.而是struct bus_type会包含struct
  device. device.bus的类型是struct bus_type.
- 在struct device结构体里没有struct device_attribute类型的成员,但是struct bus_type有.
  
  在struct driver结构体里没有struct driver_attribute类型的成员,但是struct bus_type有.
- ldd3:bus_type结构中的dev_attrs成员,指白一个为 _每个_ 加入总线的设备建立的默认属性链表.
- struct bus_type里有struct subsystem,但struct device里没有.
- device.parent:ldd3:设备的"父"设备--指的是该设备所属的设备.在大多数情况下,一个父设备通常是
  某种 _总线_ 或者是宿主控制器.如果parent是NULL,表示该设备是顶层设备,但这种情况 _很少_ 出
  现.
  
  parent可以指向总线,总线可以用struct device表示.parent为NULL很少.
- device.kobj:ldd3:表示该设备并把它连接到结构体系中的kobject.请注意,作为一个通用准
  则,device.kobj.parent与&device.parent.kobj是相同的.
  
  kobject.parent是对目录结构有影响的.
- ldd3:单纯用device结构表示的设备是很少见的,而是通常把类似kobject这样的结构内嵌在设备的高层
  表示之中.如果读者阅读pci_dev或者usb_device结构的定义,就会发现其中隐藏了device结构.
  
  但是struct cdev没有,cdev有什么特别之处吗?
- device_driver.name:ldd3:是驱动程序的名字(在sysfs中显示),那么这个各字是以一个目录还是文件存在
  呢?
  
  device_driver.bus:ldd3:是该驱动程序所操作的总线类型.
  
  device_driver.devices:ldd3:是当前驱动程序能操作的设备链表.
    
  device.driver:注释:which driver has allocated this device .
  
  所以一个设备只能由一个驱动所驱动,但是一个驱动程序可以驱动多个设备.
  
- ldd3:bus_type结构包含一个成员(drv_attrs),它指向一组为属于该总线的所有设备创建的默认属性.
  
  看来,kobject,kset,subsystem,device,driver,bus_type的成员要一个一个记住啊.
  
  发现关于结构体成员的描述还是ulk说得好点.

- ulk:Even the subsystem data structure can be embedded in a larger "container" object;
  the reference counter of the container is thus the reference counter of the embedded
  subsystemthat is, the reference counter of the kobject embedded in the kset embedded in
  the subsystem. The subsys_get( ) and subsys_put( ) functions respectively increase and
  decrease this reference counter.
  
  subsys_get()->kset_get()->kobject_get(),最终是增加了subsystem.kset.kobj.kref.refcount.

- subsystem 嵌入的是 _kset_,
  
  kset      嵌入的是 _kobject_,
  
  bus_type  嵌入的是 两个 _kset_, 一个 _subsystem_
  
  device    嵌入的是 kobject
  
  device_driver 嵌入的是 bus_type, kobject

- cdev里只有kobject,没有subsystem,kset,bus_type,device,driver.
  
  pci_dev里没有kobject,有device
  
  是不是有device就没有必要有kobject了呢?因为cdev不需要用到device是的成员来描述设备,所以就
  没有必要有device了,但是又必须有一个内嵌的kobject,且因没有devcie而不能使用device里的
  kobject,所以只好在cdev是内嵌一个kobject了.这样解释对吗?

- device.children是子设备链表的表头,device.node指向孩子链表的下一个元素.但是仅从这两个成员
  来看是不知道父子关系的,所以要用device里的kobject来表示设备的父子关系,这样/sys/devices目
  录下的目录结构才符合设备的物理组织.
  
  所有设备对象全收集到devices_subsystem中,与/sys/devices目录关联.

- device.driver_list:Each driver keeps a list of device objects including all managed
  devices; the driver_list field of the device object stores the pointers to the adjacent
  elements, while the driver field points to the descriptor of the device driver.
- device.bus_list:For each bus type, moreover, there is a list including all devices that
  are hosted on the buses of the given type; the bus_list field of the device object
  stores the pointers to the adjacent elements, while the bus field points to the bus type
  descriptor.
- struct device对象在sysfs里表现是一个目录还是一个文件呢?
- device有 _bus_id_,
  
  device_driver有 _name_,
  
  bus_type有 _name_,
- 总线有总线的属性: struct bus_attribute,
  
  设备有设备的属性: struct device_attribute,
  
  驱动有驱动的属性: struct driver_attribute.
  
  这种结构体都有struct attribute,show(),store(),不同的是show(),store()的参数类型不一样.
  
  bus_type是有bus_attribute,device_attribute,driver_attribute类型的指针.
  
  而device和device_driver都没有这些结构体成员.
- ulk:因为设备的kobject目录已经在/sys/devices/下,所以每种总线子系统的devices目录存放了指
  向/sys/devices下目录的符号链接.
- device_driver的方法成员有:suspend(),resume(),probe(),remove(),shutdown(),
  
  bus_type的方法成员有     :suspend(),resume(),match(),hotplug().
  
  它们之间有什么区别呢?
  
  bus_type.match():ulk:当内核检查一个给定的设备是否可以由给定的驱动程序处理时,就会执行
  match方法.
  
  bus_type.hotplug():ulk:在设备驱动程序模型中注册某个设备时会执行hotplug方法.
- 类由struct class描述, 每个class对象包含一个代表一个属于该class的逻辑设备的class_device描
  述符链表.
  
  就是说class_device是用来表示逻辑设备的,什么是逻辑设备呢?
  
  ulk:The class_device structure includes a dev field that points to a device descriptor,
  thus a logical device always refers to a given device in the device driver
  model. However, there can be several class_device descriptors that refer to the same
  device. In fact, a hardware device might include several different sub-devices, each of
  which requires a different User Mode interface. For example, the sound card is a
  hardware device that usually includes a DSP, a mixer, a game port interface, and so on;
  each sub-device requires its own User Mode interface, thus it is associated with its own
  directory in the sysfs filesystem.
  
- 好像bus_type,device,device_driver里都没有成员与class关联的,只是class_device里有一个
  struct device的dev成员.
  
  struct class是有一个包含一个struct class_device链表的.

** int request_resource(struct resource *root, struct resource *new)
*** kernel/resource.c:
- 这个函数主要是调用__request_resource()
...
- 有三种情况会返回第一个参数,
  ...
  #+BEGIN_EXAMPLE
	if (end > root->end)
		return root;
  #+END_EXAMPLE
- 这个函数返回NULL表示成功,否则返有冲突的resource
- 资源树可能是这样的,struct resource.child只是指向一个struct resource,然后用
  resource.sibling把所有的孩子给连起来
- for循环里的代码是如何保证请求的区间在两个区间之间又不冲突时成功的呢?
  
  保证请求区间的起始地址大于前一个区间的结西地址是for()后面的代码
  #+BEGIN_EXAMPLE
		p = &tmp->sibling;
		if (tmp->end < start)
			continue;
  #+END_EXAMPLE
  因为continue之后会处理下一个区间,所以就会用下面的代码来判断请求区间的结束地址小于当前区
  间的起始地址
  #+BEGIN_EXAMPLE
  ...
  #+END_EXAMPLE
  因为在函数的开始就处理了请求的区间有没有超过root的区间.
- 通过把新的区间的resource的插入代码来看,sibling里的resource是按照资源的位置大小来排的
  #+BEGIN_EXAMPLE
			new->sibling = tmp;
			*p = new;
  #+END_EXAMPLE
** static int find_resource(struct resource *root, struct resource *new, unsigned long size, unsigned long min, unsigned long max, unsigned long align, void (*alignf)(void *, struct resource *, unsigned long, unsigned long), void *alignf_data)
*** kernel/resource.c:
- 成功就返回0,失败就返回EBUSY
- 函数find_resource()用于在一颗资源树中寻找未被使用的、且满足给定条件的（也即资源长度大小为
  size，且在[min,max]区间内）的资源。
- for()循环的的开始先把被分配区间(new)定位在没有被使用区间内
  #+BEGIN_EXAMPLE
  ...
  new.start定好之后就用请求的大小来确定new.end
  #+BEGIN_EXAMPLE
  ...
  #+END_EXAMPLE
- 这个函数只是在root里找到合适的区间来初始化new,并没有把new插入到root的孩子链表里.
...
*** kernel/resource.c:
- 这个函数先用find_resource()找出落在某区间的指定大小的区间,
  
  接着再调用__request_resource(),若find_resource()成功了,那么调用__request_resource()一定
  是成功,调用__request_resource()的目的只是把区间插到root孩子链表里.

- 这个函数与request_resource()的区别就是这个函数指定了在某个区间里找一个指定大小的区间,并
  指定的对齐大小,
  
  request_resource()就比较死,区间就只能在指定范围里.
** #define request_region(start,n,name)	__request_region(&ioport_resource, (start), (n), (name))
*** include/linux/ioport.h:
- 这个函数只分配ioport_resource资源树里的资源.
** struct kobject * kobject_get(struct kobject * kobj)
*** lib/kobject.c:
- 增加kobject.kref.refcount, struct kref就只有refcount这一个结构体而已
** int kobject_register(struct kobject * kobj)
*** lib/kobject.c:
- 调用kobject_init()
- 调用kobject_add(),返回之后就对参数kobj创建了dentry和inode
** void kobject_init(struct kobject * kobj)
*** lib/kobject.c:
- 设置kobject.kref.refcount为1.
- 把调用kset_get()的返回值给kobject.kset.
  
  若该kobject是由某个kset.kobj指向的,那么kset_get()就会返回这个kset.
  
  要kset_get()返回kset,那么kobject.kset就不能为空
  
  但从kset_get()里调用的to_kset()来看,只是兜了一个圏而已,过程只是增加了
  kobject.kset.kobj.kerf.refcount
  
  调用kset_get()时传入的是kobject.kset, 在kset_get()里调用to_kset()时作为参数的是传给
  kset_get()的kset的kobj. 而在to_kset()里返回的是包含传入的kobject的kset,而这个kset就是传
  入给kset_get()的那个kset.所以kset_get()返回的就是传入的那个kset,而不管kset.kobj是不是与
  调用kset_get()时的kobject一样.
- 关于kset里的kobj:ulk:The kobj field is a kobject embedded in the kset data structure;
  the parent field of the kobjects contained in the kset points to this embedded
  kobject. Thus, a kset is a collection of kobjects, but it relies on a kobject of higher
  level for reference counting and linking in the hierarchical tree. This design choice is
  code-efficient and allows the greatest flexibility. For instance, the kset_get( ) and
  kset_put( ) functions, which increase and decrease respectively the reference counter of
  the kset, simply invoke kobject_get( ) and kobject_put( ) on the embedded kobject;
  because the reference counter of a kset is merely the reference counter of the kobj
  kobject embedded in the kset. Moreover, thanks to the embedded kobject, the kset data
  structure can be embedded in a "container" object, exactly as for the kobject data
  structure. Finally, a kset can be made a member of another kset: it suffices to insert
  the embedded kobject in the higher-level kset. 
  
  kset.kobj.parent指向kset.kobj,就是指向自已.
- 目录文件与kobject的关系:ulk:The directory associated with a kobject always appears in
  the directory of the parent kobject. For instance, the directories of kobjects included
  in the same kset appear in the directory of the kset itself. 
  
  注意是parent kobject. included in the same kset的意思是不是指kset.kobj.entry链起来的
  kobject呢?
- 接下来调用kobject_add()
** int kobject_add(struct kobject * kobj)
*** lib/kobject.c:
- 这个又调用kobject_get(),在kobject_init()里已经get一次了.
- 会在这里把kobject.name的地址给kobject.k_name,所以在register一个kobject的时候要初始化好
  kobject.name,好像也可以初始化好kobject.k_name,只是要分配空间.
- 接下来就是对kobject.parent来get一次.
  
  在这里得到的parent就是kobject.parent, 但是为什么在下面又把parent赋给kobject.parent呢?不
  是多此一举吗?
  
  不是多此一举的,因为在中间还一个对kobject.parent的处理,就是若kobject.kset不为 _空_,且
  kobject.parent为 _空_,那么parent就为kobject.kset.kobj.
  
  我觉得kobject.parent都是指向kobject.kset.kobj的
- device.parent:ldd3:设备的"父"设备--指的是该设备所属的设备.在大多数情况下,一个父设备通常是
  某种 _总线_ 或者是宿主控制器.如果parent是NULL,表示该设备是顶层设备,但这种情况 _很少_ 出
  现.
- 若kobject.kset不为空,那么就把kobject.entry插入到kobject.kset.list链表里.而不管这个
  kobject.parent指的是谁,反正就一定会把kobject.entry插到kobject.kset.list里.
- 调用create_dir()来创建文件
- 这个函数主要是做了3件事:
...
*** lib/kobject.c:
- 只有在kobject.k_name不为空时才可以创建文件
- 接着就是调用sysfs_create_dir()
** int sysfs_create_dir(struct kobject * kobj)
*** fs/sysfs/dir.c:
- 这个函数先找到参数kobj将被创建的位置的父目录.
  #+BEGIN_EXAMPLE
	if (kobj->parent)
  ...
  若有kobj.parent,那么就在这个parent的目录下创建.
  
  若没有kobj.parent,那么就在sysfs文件系统的最顶层去创建.
  
  所以目录说位置与kset没有太大的关系.

- 找到父目录的dentry之后,就调用create_dir()来创建一个文件
...
#define S_IXUGO		(S_IXUSR|S_IXGRP|S_IXOTH)
  #+END_EXAMPLE
- 这个函数创建的只是目录, 没有创建普通文件的功能.
- 先调用sysfs_get_dentry()在dentry cache里找看有没有这个dentry object,若没有也会创建一个的.
  
  若sysfs_get_dentry()在dentry cache里若没有找到dentry object,就会调用d_alloc()分配一个,分
  配过程中会把dentry.d_parent给初始化为sysfs_get_dentry()传入的那么parent dentry的.
- 接着调用sysfs_create()
- dentry.d_fsdata: Filesystem-dependent data
- 调用sysfs_create()创建完inode之后就调用sysfs_make_dirent()为新的inode创建新的struct
  fsdata_dirent.
- 接着就增加dentry.d_inode.i_nlink. i_nlink :Number of hard links
- 在创建struct sysfs_dirent没有错时设置dentry.d_op为sysfs_dentry_ops.
- sysfs_create()返回EEXIST时表明inode不用创建了,因为已经存在,若是其它的错,就要d_drop()掉dentry.
  #+BEGIN_EXAMPLE
...
*** fs/sysfs/inode.c:
- 这个函数先为dentry创建一个inode,若发现这个inode已经存在,那么就返回EEXIT错误码
- 调用sysfs_new_inode()来创建一个inode,这个inode特别的地方就是
  
  i_mode 为 S_IFDIR| S_IRWXU | S_IRUGO | S_IXUGO,就是说一定是一个 _目录_,所有用户可 _读_,
  所有用户可 _执行_, 只有 root可 _写_.
  
  i_mapping.a_ops为sysfs_aops
  
  i_mapping.backing_dev_info为sysfs_backing_dev_info
- 分配好inode之后就调用参数函数init(),这个函数的在create_dir()传进来的是init_dir()这个函数,这
  个函数就是设i_op为sysfs_dir_inode_operations, i_fop为sysfs_dir_operations, 递增了i_nlink.
- 最后就是调用d_instantiate(),
  
  把dentry.d_alias放到inode.i_dentry里
  
  设置dentry.d_inode为inode
- 所以这个函数任务就是给dentry分配一个对应的inode,并初始化它.
** int sysfs_make_dirent(struct sysfs_dirent * parent_sd, struct dentry * dentry, void * element, umode_t mode, int type)
*** fs/sysfs/dir.c:
- 调用sysfs_new_dirent()给新的dentry创建新的struct sysfs_dirent
  
  新的sysfs_dirent增加了sysfs_dirent.s_count
  
  新的sysfs_dirent.s_sibling插到了parent的sysfs_dirent.s_childrent里了
  
  新的sysfs_dirent.element赋予了dentry对应的的kobject.

- sysfs_new_dirent()返回之后继续初始化sysfs_dirent.
  
  s_mode为建inode时用的mode
  
- 子目录的dentry.d_fsdata与父目录的一样,因为在create_dir()里传入的fsdata就是父目录的d_fsdata.
...
*** lib/kobject.c:
- 这个函数调用对kset.kobj调用了kobject_init().
** int kset_add(struct kset * k)
*** lib/kobject.c:
- 这个函数先设置好kset.kobj.parent,
  
  若kset.kobj.parent 和 kset.kobj.kset 都为空, 且kset.subsys不为空,那么就设置
  kset.kobj.parent为kset.subsys.kset.kobj
  
  结合紧接着调用的kobject_add()关于处理kset.kobj.parent的过程,可以得出:
  
  若kset.kobj.parent为空,而kset.kobj.kset不为空,那么 _kset.kobj.parent =
  kset.kobj.kset.kobj_
  
  若kset.kobj.parent为空,而kset.kobj.kset为空,但kset.subsys不为空,那么 _kset.kobj.parent =
  &kset.subsys.kset.kobj_ 
  
  注意这个设置kset.kobj.pareent的优先级:kset.kobj.kset.kobj,   &kset.subsys.kset.kobj
  
  
- 最后对kset.kobj调用kobject_add()创建目录
...
- 还设置subsystem.kset.subsys为参数.
** void subsystem_init(struct subsystem * s)
*** lib/kobject.c:
- 对subsystem.kset调用kset_init()
** int sysfs_create_file(struct kobject * kobj, const struct attribute * attr)
*** fs/sysfs/file.c:
- 这个函数直接调用sysfs_add_file()
- 这个函数在kobj.dentry指向的目录里创建attr所描述的属性文件
** int sysfs_add_file(struct dentry * dir, const struct attribute * attr, int type)
*** fs/sysfs/file.c:
- 在kobject_add()->create_dir()->sysfs_create_dir()->create_dir()->sysfs_make_dirent(),
  kobject_add()最终调用的sysfs_make_dirent()的第二个类型为struct dentry *的参数一定不是空
  的.第二个参数dentry的来源是在第二个调用create_dir()时调用sysfs_get_dentry()分配的,调用
  sysfs_get_dentry()的返回值就是sysfs_make_dirent()的第二个参数,sysfs_get_dentry()的第一个
  参数struct dentry是sysfs_get_dentry()第二个参数的父目录的dentry,调用的
  sysfs_make_dirent()的最后一个参数是SYSFS_DIR.
  
- 这个函数只是调用sysfs_make_dirent(),第二个参数是NULL,为什么呢?
  
  http://linux.chinaunix.net/techdoc/system/2008/07/22/1020032.shtml
  其他文件系统像ext3格式中普通文件的inode，在文件创建之时就已经创建了。但是，sysfs 不一样，
  它在创建普通文件时，只是先创建一个sysfs_dirent结构。创建inode的工作是推迟到lookup函数来完
  成的。
  
  所以在调用sysfs_make_dirent()创建普通文件的sysfs_dirent时第二个参数是空的,而在创建目录的
  sysfs_dirent时候不是空的.
  
  同时也可以知道无论是普通文件还目录都会有对应的sysfs_dirent的.
- sysfs_dirent.type有五种
  值:SYSFS_ROOT,SYSFS_DIR,SYSFS_KOBJ_ATTR,SYSFS_KOBJ_BIN_ARRT,SYSFS_KOBJ_LINK
- 在create_dir()里调用sysfs_make_dirent()创建目录的sysfs_dirent时,第三个参数是kobject,是调
  用kobject_add()的参数.
  
  在sysfs_add_file()调用syfs_make_dirent()创建普通文件的sysfs_dirent时,第三个参数是struct
  attribute,
  
  现在有一个问题:struct attribute只有三个参数:name,owner,mode.那么在为一个属性创建一个文件
  时,它的内容是如何确定的呢?
...
struct sysfs_ops {
  ...
  ldd3:当用户空间读取一个属性时,内核会使用指向kobject的指针和正确的属性结构来调用show方法.该
  方法将把指定的值编码后放入缓冲区,然后把实际长度做为返回值返回.
  
  ldd3:也可以对所有的kobject的属性使用同一个show方法.传递给该函数的attr指针可以用来判断所
  请求的是哪个属.一些show方法包含一系列对属性名的检查.其他的实现方法会把attribute结构嵌入
  到其他结构中,而在那些结构中包含了需要返因的属性值信息.
  
  第一种使用attribute的方法就是在一个show()里通过判断attribute.name来确定访问的是那个属性,
  从而确定操作.
  
  第二种使用attribute的方法就是在一个show()里通过contain_of()真正包含attribute的结构体
  (kobj_type.default.attrs是一个指针的指针),而在这个结构体里就有要访问的内容.
- 现在发现一个问题:在创建目录的时候,传给sysfs_make_dirent()的第二个参数dentry为不空,所以可
  以初始化dentry.d_fsdata为刚分配的sysfs_dirent.
  
  但是如果是普通文件,第二个参数为NULL,只有在用syfs_lookup()查找时才创建,那么它的
  syfs_dirent又是如何被保存起来的呢?
  
  应该是在sysfs_make_dirent()的开头调用的sysfs_new_dirent()时保存的.保存在了
  parent_sd.s_children链表里.
  
  的确是这样的,因为在sysfs_lookup()里遍历了parent_sd.s_children
  
  #+BEGIN_EXAMPLE
	list_for_each_entry(sd, &parent_sd->s_children, s_sibling) {
  #+END_EXAMPLE 
  
  在sysfs_lookup()里通过判断传入的dentry的d_name.name是否与sysfs_dirent的名字是否相等来配
  匹.sysfs_dirent获得它的名字的方法是先通过s_type来确定获取名的方式,
  
  SYSFS_DIR:名字是s_dentry.d_name.name.
  
  SYSFS_KOBJ_ATTR:s_element是struct attribute类型,名字是s_element.name
  
  还有SYSFS_KOBJ_BIN_ATTR,SYSFS_KOBJ_LINK类似SYSFS_KOBJ_ATTR.
- 如果是创建普通文件的话,文件的属性是与attribute.mode有关的
  #+BEGIN_EXAMPLE
...
#define S_IALLUGO	(S_ISUID|S_ISGID|S_ISVTX|S_IRWXUGO)
  ...
  #+END_EXAMPLE
- sysfs_make_dirent()的所有参数就足够初始化sysfs_dirent.
  
  如在sysfs_add_file()里调用的
  #+BEGIN_EXAMPLE
...
** not function
*** 13.3
- As far as the kernel is concerned, the name of the device file is irrelevant. If you
  create a device file named /tmp/disk of type "block" with the major number 3 and minor
  number 0, it would be equivalent to the /dev/hda device file shown in the table. 
  
  不同的设备名可以有相同的主次设备号.
*** 13.5
- cdev.list/count:ulk:The list field is the head of a doubly linked circular list
  collecting inodes of character device files that refer to the same character device
  driver. There could be many device files having the same device number, and all of them
  refer to the same character device. Moreover, a device driver can be associated with a
  range of device numbers, not just a single one; all device files whose numbers fall in
  the range are handled by the same character device driver. The size of the range is
  stored in the count field.
  
  因为不同的设备文件有相同的设备号,这些有相同设备号的文件的inode会链到 _cdev.list_ 上,通过
  _inode.i_devices_ 链起来.
  
  除了相同之外,在 _cdev.count_ 指定范围之内的设备号也会链起来.
  
  
** struct cdev *cdev_alloc(void)
*** lib/kobject.c:
- 这个函数动态分配一个cdev,并初始化里面的kobject
- 没有struct cdev的slab.
- 初始化cdev.kobj.ktype为ktype_cdev_dynamic,
  
  ktype_cdev_dynamic只有一个release成只,它初始化为cdev_dynamic_release.
  
  cdev_dynamic_release()
  (1)把cdev.list里的inode都解除链接,但没有释放inode.
  #+BEGIN_EXAMPLE
...
- 接着就调用kobject_init()
  ...
  调用kref_init()增加kobject.kref.count计数器.
  
  调用kset_get()
  #+BEGIN_EXAMPLE
	kobj->kset = kset_get(kobj->kset);
  #+END_EXAMPLE
  其实kobj.kset是空的,所以这句其实无效.
- 所以这个函数就是
  
  分配了cdev
  
  初始化cdev.kobj.ktype(release方法)
  
  增加cdev.kobj.kref.count
** int cdev_add(struct cdev *p, dev_t dev, unsigned count)
*** fs/char_dev.c:
- ulk:The cdev_add( ) function registers a cdev descriptor in the device driver model.
- 初始化cdev.dev为参数dev,
  
  初始化cdev.count为参数count.
- 调用kobj_map().
...
- The device driver model defines a kobject mapping domain for the character devices,
  which is represented by a descriptor of type kobj_map and is referenced by the cdev_map
  global variable. The kobj_map descriptor includes a hash table of 255 entries indexed by
  the major number of the intervals. The hash table stores objects of type probe, one for
  each registered range of major and minor numbers

  kobj_map是设备驱动模型给 _字符_ 设备定义的一个 kobject mapping domain.
  
  cdev_map是struct kobj_map *类型的全局变量.
  
  在cdev_add()里调用kobj_map()的时候传的kobj_map是cdev_map
  #+BEGIN_EXAMPLE
	return kobj_map(cdev_map, dev, count, NULL, exact_match, exact_lock, p);
  #+END_EXAMPLE 
- kobj_map结构体里有一个类型为struct probe *[255]的成员,这个指针数组的元素是一个hash链表.
  这些链表把struct probe给链起来.
  #+BEGIN_EXAMPLE
		p->next = *s;
		*s = p;
  #+END_EXAMPLE 
- 这个函数要注意两点:
  ...
  次设备号的个数可以超过了最大值,所以可能跨过一个或多个主设备号,但是这不是错误.
  
  kobj_map.probes里的每个hanh链表内部结点的排序是按照probe.range从小到大来的.
- 先找出range跨过多少个主设备号
  #+BEGIN_EXAMPLE
  ...
  所以要分配合适个数的struct probe
  #+BEGIN_EXAMPLE
	p = kmalloc(sizeof(struct probe) * n, GFP_KERNEL);
  #+END_EXAMPLE 
  
  虽然是分配了多个struct probe,但是每一个的初始化都是一样的
  #+BEGIN_EXAMPLE
	for (i = 0; i < n; i++, p++) {
		p->owner = module;
		p->get = probe;
  ...
  cdev_add()调用这个函数时传入的moudle是NULL,为什么不传入cdev.owner呢?
  
  probe.get:ulk:the get method is implemented by a short function that returns the address
  of the kobject embedded in the cdev descriptor;
  probe.get对于cdev来说就是返回内嵌到cdev里的 _kobject的地址_.
  
  probe.lock:ulk:the lock method, instead, essentially increases the reference counter in
  the embedded kobject.
  增加kobject.kref.count.
  
  结合cdev_add()的调用来看,probe.dev就是起始的主次设备号结合的类型为dev_t设备号.
  
  probe.range就是次设备号数.
  
  结合cdev_add(),probe.data就是cdev
- 最后把所有新建的probe插入到hash表
  #+BEGIN_EXAMPLE
...
*** drivers/base/map.c
- ulk:The kobj_lookup( ) function receives as input parameters a kobject mapping domain
  and a device number; it searches the hash table and returns the address of the kobject
  of the owner of the interval including the number, if it was found. When applied to the
  mapping domain of the character devices, the function returns the address of the kobject
  embedded in the cdev descriptor of the device driver that owns the interval of device
  numbers.
- kobj_map.probe的hash表是根据主设备号来计算hash值的,而不是设备号
  #+BEGIN_EXAMPLE
...
- 这个函数定位到hash表之后
  ...
  在该链表里找到第一个kobj_map的设备范围在包含参数dev的kobj_map结点.
  #+BEGIN_EXAMPLE
		if (p->dev > dev || p->dev + p->range - 1 < dev)
			continue;
  #+END_EXAMPLE 
  
  找到之后就调用lock(),这个方法对于cdev来说是增加kobject成员的计数.
  #+BEGIN_EXAMPLE
  ...
  调用完lock(),再调用probe().对于cdev来说,就是获取cdev.kobj.
  #+BEGIN_EXAMPLE
...
- 有些细节要处理
  ...
  希望能在释放信号量后调用probe(),所以得到kobj_map之后就是保存kobj_map的成员到局部变量以释
  放信号量再用
  #+BEGIN_EXAMPLE
		owner = p->owner;
		data = p->data;
  ...
  index是返回给调用者的,意思是获得是kobj_map的dev与传入的dev的差值,就是索引.
  
  best不知有什么用.
** not function
*** 13.5.1
- struct char_device_struct,这个结构体用来表示一个设备号区间.
  
  用3个成员:major,baseminor,minocrt

  major:The major number of the interval
  
  baseminor:The initial minor number of the interval
  
  minocrt:the interval size
  
  就是主设备号, 起始次设备号, 次设备号个数.
  
  因为所有的char_device_struct是被链到chrdevs hash表里的,所以要用 _next_ 成员用来链接.
  
  name成员:使用这个区间的 _驱动_ 的名字.
  
  cdev成员:指向设备驱动描述符.
  
  hash链表里的结点是按设备号由小到大排序的.
** int register_chrdev_region(dev_t from, unsigned count, const char *name)
*** fs/char_dev.c:
- 这个函数对在参数dev+count范围内设备号所涉及的所有 _主设备号_ 调用__register_chrdev_region()
- 这个函数注册的设备号范围是这样的:
  
  主设备号从参数from这个设备号指的主设备号开始, 一直到from+count这个设备号设表示的主设备号
  结束.
  
  次设备号除了第一个要被注册的主设备号(最小的主设备号)外, 其它被注册的所有主设备号的从0到
  一个主设备号所能关联的最大次设备号都被注册.第一个被注册的主设备号的次设备号范围是个参数
  from这个设备号的次设备号到最大次设备号.
 ...
 * Register a single major with a specified minor range.
  #+END_EXAMPLE
- 参数major为0的时候会动态分配一个主设备号.
  
  但是major为0时分配主设备号的方法不太好,只分配255个hash表里所有为 _空_ 的hash表的 _最大索
  引_ 的那个hash表作为分配的major,所以分配的major不会大于255.
  
  ulk:Notice that the kernel can dynamically allocate only major numbers less than 255,
  and that in some cases allocation can fail even if there is a unused major number less
  than 255. We might expect that these constraints will be removed in the future.
  ...
  #+END_EXAMPLE
- 接下来就是初始化除了char_device_struct.cdev之处的所有成员.
  #+BEGIN_EXAMPLE
...
- 所以这个函数就是分配一个char_device_struct并初始化它.
** int alloc_chrdev_region(dev_t *dev, unsigned baseminor, unsigned count, const char *name)
*** fs/char_dev.c:
- 这个函数也调用__register_chrdev_region(),
  
  与register_chrdev_region()不同的是没有处理所有次设备号超范围的情况.
  
  且是用动态分配主设备号的.
** int register_chrdev(unsigned int major, const char *name, struct file_operations *fops)
*** fs/char_dev.c:
- The register_chrdev( ) function is used by drivers that require an old-style interval of
  device numbers.
- 这个函数也调用__register_chrdev_region(),但主设备号是 _静态_ 分配的,且次设备是 0-255.
- 和register_chrdev_region()和alloc_chrdev_region()不同的是 还分配了cdev并调用cdev_add().
** int chrdev_open(struct inode * inode, struct file * filp)
*** fs/char_dev.c:
- 这个函数是对字符设备调用open()时调用的.
- 首先看inode.i_cdev是否为空,为空就说明inode还没有访问过,所以要初始化i_cdev
  #+BEGIN_EXAMPLE
	if (!p) {
		struct kobject *kobj;
		int idx;
  ...
  接着就是初始化i_cdev, i_cindex.i_devices
  i_cindex: Index of the device file within a group of minor numbers.
  #+BEGIN_EXAMPLE
			inode->i_cdev = p = new;
			inode->i_cindex = idx;
			list_add(&inode->i_devices, &p->list);
  #+END_EXAMPLE 
- 搞定了i_cdev之后,就是初始化f_op,
  #+BEGIN_EXAMPLE
	filp->f_op = fops_get(p->ops);
  #+END_EXAMPLE 
  f_op直接就是cdev.ops.
- 最后就是调用f_op.open了. 真正调用cdev.ops.open是在这里,之前经过了很多的处理.
** not function
*** 14
- Block devices have very high average access times. Each operation requires several
  milliseconds to complete, mainly because the disk controller must move the heads on the
  disk surface to reach the exact position where the data is recorded.

  几个微秒就可以寻到址.
- 从图14-1来看,一个块设备驱动程序涉及的内核部分里有3个这样的层:mapping layer,接下来是
  generic block layer, 接丁来是 I/Oscheduler layer.
- ulk:3. Let's assume that the kernel must read the data from the block device, thus it
  must determine the physical location of that data. To do this, the kernel relies on the
  mapping layer , which typically executes two steps:
  
  a. It determines the block size of the filesystem including the file and computes the
  extent of the requested data in terms of file block numbers . Essentially, the file is
  seen as split in many blocks, and the kernel determines the numbers (indices relative to
  the beginning of file) of the blocks containing the requested data.
  
  b. Next, the mapping layer invokes a filesystem-specific function that accesses the
  file's disk inode and determines the position of the requested data on disk in terms of
  logical block numbers. Essentially, the disk is seen as split in blocks, and the kernel
  determines the numbers (indices relative to the beginning of the disk or partition)
  corresponding to the blocks storing the requested data. Because a file may be stored in
  nonadjacent blocks on disk, a data structure stored in the disk inode maps each file
  block number to a logical block number.
  
  mapping layer主要是用文件的 _逻辑块号_ 获得数据在磁盘的信置.
- 4. The kernel can now issue the read operation on the block device. It makes use of the
  generic block layer , which starts the I/O operations that transfer the requested
  data. In general, each I/O operation involves a group of blocks that are adjacent on
  disk. Because the requested data is not necessarily adjacent on disk, the generic block
  layer might start several I/O operations. Each I/O operation is represented by a "block
  I/O" (in short, "bio") structure, which collects all information needed by thelower
  components to satisfy the request.
  
  所以generic block layer主要是发送I/O请求(bio),每次的请求只能作用于相邻的disk块.
- 5. Below the generic block layer, the "I/O scheduler " sorts the pending I/O data
  transfer requests according to predefined kernel policies. The purpose of the scheduler
  is to group requests of data that lie near each other on the physical medium. 
  
  I/O scheduler层保存未决的数据请求,主要目的是相邻的数据请求给组织起来.
- 要分清: 扇区(sector), 块(block), 段(segment), 页(page).
  
  ulk:there are many kernel components that are concerned with data stored in block
  devices; each of them manages the disk data using chunks of different length: 
  
  The controllers of the hardware block devices transfer data in chunks of fixed length
  called "sectors." Therefore, the I/O scheduler and the block device drivers must manage
  sectors of data.

  The Virtual Filesystem, the mapping layer, and the filesystems group the disk data in
  logical units called "blocks." A block corresponds to the minimal disk storage unit
  inside a filesystem. 
  
  As we will see shortly, block device drivers should be able to cope   with "segments" of
  data: each segment is a memory pageor a portion of a memory pageincluding chunks of data
  that are physically adjacent on disk.

  The disk caches work on "pages" of disk data, each of which fits in a page frame. The
  generic block layer glues together all the upper and lower components, thus it knows
  about sectors , blocks, segments, and pages of data.
  
  块设备的驱动器以 _sector_ 的长度传输数据. 所以 _I/O scheduler_ 和 _block device driver_
  要知道sector的长度.
  
  _virtual filesystem_ 和 _mapping layer_ 处理的是 _block_, block是filesystem是最小存储单
  位.
  
  _segment_ 与 _驱动_ 有关,驱动能以segment来拷贝, segment的数据在物理磁盘上是 _连续_ 的且
  是一个 _页_ 或一个页的 _部分_.
  
  因为 _generic block layer_ 在中间,所以它知道 _sector_, _block_, _segment_, _page_.
- 图14-2给出了典型的block, segment, sector, page的大小
  ...
  page 含 segment 含 block 含 sector
- 大部分block device的sector大小是512B
- ulk:In Linux, the size of a sector is conventionally set to 512 bytes; if a block device
  uses larger sectors,the corresponding low-level block device driver will do the
  necessary conversions.
- 一个block的数据可能在disk里不是连续的sector.
- ulk:In Linux, the block size must be a power of 2 and cannot be larger than a page
  frame. Moreover, it must be a multiple of the sector size,
  
  block要小于page, 因为page要可以分成page buffer
  
  block是2的幂次大小
  
  block是sector的倍数.
- 一次的DMA操作所涉及说sector必须在disk上是连续的.
- 14章讨论generic block layer层及以下的 I/O scheduler 和 block device.
  
  没有讨论virtual filesystem 和 mapping layer.
- generic block layer这一层使得
  
  用户态的数据可以map/unmap到内核态.
  
  disk和用户态空间可以直接交互,不用通过内核.
  
  提供给LVM和RAID来管理逻辑卷,不同的多个的partitions可以看成一个partition.
  
  优化disk控制器的功能.
- generic block layer的核心数据结构是struct bio.
  
  因为generic block layer要知道page, segment, block, sector.所以当然要知道segment.
  
  表示segment的数据结构是struct bio_vec.

  在bio里有指向bio_vec链表第一个bio_vec的指针叫bi_io_vec.
- 在I/O操作时要跟踪更新bio,如scatter-gather DMA操作可能不能在一次完成, 所以要用 _bi_idx_
  来跟踪当前传到第几个segment.
- 一个disk用 _struct gendisk_ 表示.
- /dev/hda和/devhad1的 _主_ 设备号是 _相同_ 的, _次_ 设备号是 _不同_ 的.
- 每个分区的一些东西用 _struct hd_struct_ 表示,gendisk.part指向struct hd_struct 的 _数组_.
** void generic_make_request(struct bio *bio)
*** drivers/block/ll_rw_blk.c:
- bio分配好并初始化之后调用这个函数,这个函数是generic block layrer的主要入口.
- 这个函数的作用:注释: is used to make I/O requests of block devices.
- 成功与否是通过bio.bi_end_io方法来 _异步_ 通知的.
- 调用这个函数前要初始化好bio:
  ...
  bi_end_io
- bd_inode: Pointer to the inode of the file associated with the block device in the bdev
  filesystem.
  
  bdev filesystem,好像第一次听这东西,bdev filesystem其实是 _特殊文件系统_ 的一种,bdev
  filesystem系统是 _没有挂载点_,还有其它的文件系统.
  
  其它的特殊文件系统ulk 的表12-8有列出,如pipefs,rootfs,shm,sockfs,tmpfs,usbs等等.
  
  ulk:the kernel assigns to each mounted special filesystem a fictitious block device that
  has the value 0 as major number and an arbitrary value (different for each special
  filesystem) as a minor number.
  
  除了 special filesystem之外,还有 _disk-based filesystem_ 和 _network filesystem_.
  
  如果文件系统是没有挂载点的,如bdev文件系统,那么文件系统的定义又是什么呢?
  
  http://zh.wikipedia.org/wiki/%E6%96%87%E4%BB%B6%E7%B3%BB%E7%BB%9F
  文件系统是一套实现了数据的存储、分级组织、存取和获取等操作的抽象数据类型（Abstract data
  type）。
- i_size:File length in bytes
  ...
  bi_bdev可能是一个disk或是partition
  
  从代码里的 _魔数_ 9可以看出,扇区的大小的固定死的了,是 _512B_.
- 检查bio里请求的扇区范围有没有超过设备的范围
  #+BEGIN_EXAMPLE
  ...
  处理错误的方法是在handle_bad_sector()里设置bi_flags的 _BIO_EOF_ 标志, 夕然后调用
  bi_end_io.
- 调用bdev_get_queue()来获得设备队列,
  
  若没有队列就认为块设备不存在,这个可以从打印信息里看出
  #+BEGIN_EXAMPLE
...
- 一个块设备队列是一个disk一个还是一个分区一个呢?应该是一个disk一个,bd_disk: Pointer to
  gendisk structure of the disk underlying this block device.
  
  bd_disk有queue成员, bd_disk是指向 _disk_ 的gendisk, 不是指向patition的gendisk.
  
  其实准确来说是一个disk controller一个队列,ulk:Each block device driver maintains its own
  request queue, which contains the list of pending requests for the device. If the disk
  controller is handling several disks, there is usually one request queue for each
  physical block device.

- bio提交的扇区数不能大于队列能处理的最大扇区数
  #+BEGIN_EXAMPLE
  ...
  #+END_EXAMPLE 
- 调用block_wait_queue_running(),ulk: to check whether the I/O scheduler currently in use
  is being dynamically replaced; in this case, the function puts the process to sleep
  until the new I/O scheduler is started.
- 调用blk_partion_remap()把bi_sector和bi_bdev转成disk的.所以插入到队列的bio请求的扇区是对
  disk的位置,而不是分区的.
- 调用队列的make_request_fn()来把bio插入到队列.
** static inline void blk_partition_remap(struct bio *bio)
*** drivers/block/ll_rw_blk.c:
- 首先看bio的请求是从分区发出的还是从disk发出的.
- bi_bdev:Pointer to block device descriptor. 所以bi_bdev有可能指向分区或disk的设备文件.
  
  bd_contains: If block device is a partition, it points to the block device descriptor of
  the whole disk; otherwise, it points to this block device descriptor
  #+BEGIN_EXAMPLE
	struct block_device *bdev = bio->bi_bdev;

	if (bdev != bdev->bd_contains) {
  #+END_EXAMPLE 
- bd_part: Pointer to partition descriptor (NULL if this block device is not a partition)
- struct hd_struct就是分区的描述符.
- 若是分区的话,就要修改

  hd_struct.read_sectors
  
  hd_struct.read
  
  hd_struct.write_sectors
  
  hd_struct.write
  
  bio.bi_sector 改为在disk的位置,而不是原来分区的位置.
  
  bio_bi_bdev   改为bd_contains.
** not function
*** 14.3
- 为了减少磁头移动次数(磁盘转而磁头不动), _内核_ 会聚集多个sector来作一次处理.
  
  ulk:Although block device drivers are able to transfer a single sector at a time, the
  block I/O layer does not perform an individual I/O operation for each sector to be
  accessed on disk; this would lead to poor disk performance, because locating the
  physical position of a sector on the disk surface is quite time-consuming. Instead, the
  kernel tries, whenever possible, to cluster several sectors and handle them as a whole,
  thus reducing the average number of head movements. 
  
  When a kernel component wishes to read or write some disk data, it actually creates a
  block device request. That request essentially describes the requested sectors and the
  kind of operation to be performed on them (read or write). However, the kernel does not
  satisfy a request as soon as it is createdthe I/O operation is just scheduled and will
  be performed at a later time. This artificial delay is paradoxically the crucial
  mechanism for boosting the performance of block devices. When a new block data transfer
  is requested, the kernel checks whether it can be satisfied by slightly enlarging a
  previous request that is still waiting (i.e., whether the new request can be satisfied
  without further seek operations). Because disks tend to be accessed sequentially, this
  simple mechanism is very effective.

  I/O操作是异步的,就是说块设备的驱动会异步读取上层发来的请求,而唤醒驱动处理下一个请求的是
  disk controller处理完一个I/O操作后所产生的中断.
  
  ulk:To keep the block device driver from being suspended, each I/O operation is
  processed asynchronously. In particular, block device drivers are interrupt-driven (see
  ...
  already existing one and then terminates. The block device driver, which is activated at
  a later time, invokes the strategy routine to select a pending request and satisfy it by
  issuing suitable commands to the disk controller. When the I/O operation terminates, the
  disk controller raises an interrupt and the corresponding handler invokes the strategy
  routine again, if necessary, to process another pending request. 
- request_queue是队列结构体, request_queue.queue_head是请求的链表头.
- 一个quest里有多个bio的厡因是:
  
  1.I/O scheduler 加一个新的 segment 到bio,
  
  2.链接另一个bio到quest
  
  当新的数据在物理上与在quest里旧的数据 _相邻_ 时就会有这些情况.
  
  _request.bio_ 指向第一个bio, _biotail_ 指向最后一个, _rq_for_each_bio_ 遍历.
- 每一个request_queue都有一个管理request内存池分配的成员 _request_list_.
  
  request_list里有 _两个_ 等待 READ和WRITE request可用的等待队列, _一个_ 等待队列刷新的等
  待队列
  
  request_list.count[2]:Two counters for the number of requests descriptors allocated
  for READ and WRITE requests,respectively.
  
  request_list.starved[2]:Two flags indicating whether a recent allocation for a READ or
  WRITE request, respectively,failed.
** struct request *blk_get_request(request_queue_t *q, int rw, int gfp_mask)
*** drivers/block/ll_rw_blk.c:
- ulk:The blk_get_request( ) function tries to get a free request descriptor from the
  memory pool of a given request queue;
- 如果有 __GFP_WAIT_ 标志,表示分配时可以等待.调用
  ...
  get_request_wait()里调用get_request()分配,若分配不到就休眠.
** static struct request *get_request(request_queue_t *q, int rw, int gfp_mask)
*** drivers/block/ll_rw_blk.c:
- request_queue.nr_requests: ulk: The field of the request descriptor stores the maximum
  number of allowed pending requests for each data transfer direction. 
  
  若超过了 nr_request ,
  
  就设置 _QUEUE_FLAG_READFULL (QUEUE_FLAG_WRITEFULL)_,
  
  并把进程放到request_list里相应等待队列里.

- nr_request是最大 request 数, (128)
  
  nr_congestion_on是拥挤阀值, (113)
  
  nr_congestion_off是不拥挤阀值. (111)
  
  blk_congestion_wait()会把进程休眠,若当前是拥挤的.

- unpluging - activating
  
  pluging   - de-activating
** void blk_plug_device(request_queue_t *q)
*** drivers/block/ll_rw_blk.c:
- 这个函数是 plug(de-active) 一个队列的.
- 不能 plug 一个停止的队列
  #+BEGIN_EXAMPLE
...
- 主要做两件事:
  ...
  设置QUEUE_FLAG_PLUGGED
  
  若QUEUE_FLAG_PLUGGED没有设置,那么就开始request_queue.unplug_timer的计时.
** int blk_remove_plug(request_queue_t *q)
*** drivers/block/ll_rw_blk.c:
- 这个函数与 _blk_plug_device()_ 对应
  
  所以做两件事:
  
  清 QUEUE_FLAG_PLUGGED,
  
  删request_queue.unplug_timer

- 这个函数在 _两_ 种情况下被调用:
  
  队列里的可合并 request 都 in sight. 这由 _内核_ 显式调用.
  
  当队列里 _未决_ 的request超过 _request_list.unplug_thres_ (4)时,由 _I/O scheduler_ 调用.
** not function
*** 14.3.3
- 当 _request_queue.unplug_timer_ 超时后,就是 _request_queue.unplug_delay_ ,
  
  就执行定时器函数 _blk_unplug_timeout()_
  
  转而唤醒服务于 _kblockd_workqueue_ 工作队列的 _kblockd_ 内核线程.
  
  kbloced 转而执行 _request_queue.unplug_work_ 方法, 一般是 _blk_unplug_work()_.
  
  blk_unplug_work() 转而执行 _request_queue.unplug_fn_, 一般是 _generic_unplug_device()_
  
  转而 _generic_unplug_device()_ 调用 _request_queue.request_fn_.
  
  总结就是 超时后唤醒线程执行 unplug_work -> unplug_fn -> request_fn.
*** 14.3.4
- 处理 request 的算法是 电梯 算法.
  
  在某个时刻磁头是向一个方向移动的(只有两个方向), 那么磁头会一直向那个方向移动直到当前在那
  个方向上的所有request都被处理到,然后再向反方向移动.
...
- 有一个 _调度队列(dispatch queue)_ 的概念, 它包含的是已经排序好的request

- request_queue.elevator的类型是elevator_t的电梯对象.里面有很多方法
- noop 算法, 它没有按顺序的队列,不排序请求.
- CFQ 算法,
  
  它保证发出请求的进程的 I/O 带宽是公平分配的.
  
  有 64 个队列, 一个request插到哪个队列是根据发request的进程的组ID来hash的结果.
  
  再用 _round_robin_ 遍历 64 个队列来选择把一个队列的一些request放到 dispatch 队列里.
- deadline elevator
  
  这个算法是消除请求饥饿的.
  
  有 _一_ 个 _dispatch queue_.
  
  有 _两_ 个 _sorted queue_, 分别是 _读_ 和 _写_ 的, 里面的 requrest 是按 _sector_ 号排序
  的.
  
  还有 _两_ 个 _deadline queue_, 也分 _读_ _写_, 按 _deadline_ 排.
  ...
  填充dispatch queue时,先确定是 _读还是写_, _读_ 优先,若 deadline queue有超时,就把第一个超
  时的request移到 dispatch queue, 再从 sorted queue 移一批次扇区号在超时request之后的
  request,越相邻就移得越多.若没有超时,就dispatch一批次以从sorted queue移的最后一个request
  开始的request.
  
  那么不是在提交request的时候,不是要指定是放在sorted queue还是放在deadline queue.
- Anticipatory elevator
  
  它是基于deadline elevator的.
  
  比deadline elevator多了两点:
  
  1.dispatch 一个在磁头当前方向的反方向的 request的情况,当在sorted queue里反方向的 request
  与当前磁头的距离比在sorted queue里正方向的 request与当前磁头的距离的 _一半_ 还要小.
  
  2.处理了进程P的 _读_ 请求之后,若sorted queue的下一个request也是P的,那么就马上处理.否则会
  根据进程P的状态来判断P是否很快发出下一个请求,若是就等一下.因同一个进程连续发出的请求的
  sector可能相邻.
** void blk_queue_bounce(request_queue_t *q, struct bio **bio_orig)
*** mm/highmem.c:
- ulk:The blk_queue_bounce( ) function looks at the flags in q->bounce_gfp and at the
  threshold in q->bounce_pfn to determine whether buffer bouncing might be required. This
  happens when some of the buffers in the request are located in high memory and the
  hardware device is not able to address them.
- request_queue.bounce_gfp: Memory allocation flags for bounce buffers
  
  bounce_gfp 设了GFP_DMA就使用page_pool内存池来调用__blk_queue_bounce()
  
  否则就用isa_page_pool.isa_page_pool内存池是用GFP_DMA分配的.
** static void __blk_queue_bounce(request_queue_t *q, struct bio **bio_orig, mempool_t *pool)
*** mm/highmem.c:
- 第一个循环的处理结果是:
  
  若有一个segment是在高端内存,就会分配一个新的bio,
  
  对旧bio所有的在高端内存的segment都移到的新的bio的segment上去,移的东西包括struct bio_vec
  是的所有成员.
  
  注意只是处理了在高端内存的segment, 没有处理 _bio的其它成员_ 和 _不在高端内存的segment_,
  而是在接下来处理.
...
- 接下来是处理剩下的segment, 因为剩下的segment是在DMA区的,所以不用进行数据拷贝,只是设置
  struct bio_vec的其它成员为旧的bio
  #+BEGIN_EXAMPLE
...
- 接下来就是初始化bio的其它成员, bi_flags要多设置BIO_BOUNCED.
  #+BEGIN_EXAMPLE
  ...
  bi_priavte保存了bio_orig的地址, 而bio_orig指向了新的bio, 这对调用者要注意,就是参数
  *bio_orig就是可以传给I/O scheduler的bio的指针.
** static int __make_request(request_queue_t *q, struct bio *bio)
*** drivers/block/ll_rw_blk.c:
- 之前讨论的generic_make_request()里调用了request_queue.make_request_fn. make_request_fn()
  一般由__make_request()实现.
- 看到这段代码想到是不是把bi_vec里所有的段的长度起来就是bi_size.
  #+BEGIN_EXAMPLE
	nr_sectors = bio_sectors(bio);
	cur_nr_sectors = bio_cur_sectors(bio);
  #+END_EXAMPLE
- 调用blk_queue_bounce()按需来建立低内存的segment,
  
  从blk_queue_bounce()来看新建的bio已由bio引用,旧的bio在bio.bi_private里.
- 调用elv_queue_empty()看dispatch queue是否为空,
  
  dispatch queue为空不代表其它的类型的queue就为空.
  ...
  #+END_EXAMPLE
- 剩下的就是看bio能否 _合并_ 到已有的 request,若可以就合并,不可以就新建一个request,并初始
  化request,把bio插进去.
  
  调用elv_merge()判断是否可以合并,若可以合并,第二个参数返回可以合并的 request. elv_merge()
  返回合并的类型.
  
  因为一个 request 里的所有bio所表示的I/O操作都是相邻, 合并bio要么是bio放到request 的bio链
  表里的 _开头_ ,要么 _尾端_ .或是 _合并两个request_.
  
  合并类型有:
  
  ELEVATOR_BACK_MERGE:bio放到尾端,
  
  ELEVATOR_FRONT_MERGE:放到开头,
- 若不能合并,就分配一个新的request,因为在分配一个request时把队列的锁给解了,所以分配完之后
  要回头执行,
  #+BEGIN_EXAMPLE
	if (freereq) {
		req = freereq;
...
** not function
*** 19.1.1
- POSIX 定义的是 _半双工_ pipe.
  
  system v release 4 是 _全双工_ pipe.
  
  linux 是 _半双工_ pipe, 但不用关闭其中一端.
- ulk:Once a pipe is created, a process uses the read( ) and write( ) VFS system calls to
  access it. Therefore, for each pipe, the kernel creates _an_ inode object plus _two_
  file objectsone for _reading_ and the other for _writing_.
- ulk:When the inode object refers to a pipe, its _i_pipe_ field points to a
  _pipe_inode_info_ structure
- ulk:The number of bytes currently written in all pipe buffers and yet to be read is the
  so-called _pipe size_.
- linux为了想使用 _大于4k_ 的pipe buffer,所以把 _16_ 个4k的页框看成了一个pipe buffer,组织这
  16个pipe buffer的方式是使用数组, _pipe_inode_info.bufs_ .
  
  pipe_inode_info.curbuf的类型是 struct pipe_buffer. 
  
  为了把这16个页框看成一个 _循环_ pipe buffer, 就要有方法获得当前 _读位置_ 和 _写位置_.
  
  读位置:先用pipe_inode_info.curbuf 确定读位置是在哪个页框上的,就是pipe_inode_info.bufs的
  _下标_, 找到页框之后就是用 _pipe_buffer.offset_ 确定在页内的偏移量.
  
  写位置:用读位置来确定写位置, 多使用一个 _pipe_inode_info.nrbuf_ 表示一共有多少个pipe
  buffer是有数据的. 所以写位置在最后一个有数据的页框的有效数据长度加1, _pipe_buffer.len_.
** asmlinkage int sys_pipe(unsigned long __user * fildes)
*** arch/i386/kernel/sys_i386.c:
- 调用do_pipe()
** int do_pipe(int *fd)
*** fs/pipe.c:
- 先调用get_empty_file() _两次_ 来获得两个 _FILE_
- 调用get_pipe_inode()获得一个inode并初始化.
- 再调用get_unused_fd() _两次_ 来获得两个 _文件描述符_.
- 调用d_alloc分配一个dentry
  #+BEGIN_EXAMPLE
  ...
  可见pipe在pipefs文件系统都是以 _inode号_ 作为名字的,
  
  且所有的pipe都是放在pipefs的 _根目录_.
** static struct inode * get_pipe_inode(void)
*** fs/pipe.c:
- 调用new_inode()从pipe_mnt.mnt_sb spuerblock里分配一个inode.
- 初始化 _inode.i_pipe.readers_ 和 _inode.i_pipe.writers_ 为1.
- 初始化 _inode.i_ops_
- inode.i_state 为I_DIRTY.
  #+BEGIN_EXAMPLE
	/*
	 ...
	 */
  #+END_EXAMPLE
- inode.i_mode设了S_IFIFO,
  
  pipe只能由建pipe的用户来读写.因为只设了 S_IRUSR和S_IWUSR
- 接下来就是初始化两个file的成员.
** not function
*** 19.1.3
- fork()不会增加readers和writers计数器, readers和writers总是1, 但会增加父进程所有file的计数,
  所以就算父进程die了,但是父进程的所有file对象没有释放, 因为计数没为0,因为子进程还在使用它
  们.
  
  ulk:Forking a new process does not increase the value of the
  readers and writers fields, so they never rise above 1;[*] however, it does increase the
  value of the usage counters of all file objects still used by the parent process (see
  the section "The clone( ), fork( ), and vfork( ) System Calls" in Chapter 3). Thus, the
  objects are not released even when the parent dies, and the pipe stays open for use by
  the children.
  
  fork()一个进程时file对象是共享的.
** not function
*** 19.1.4
- 要用fcntl()才可以设置非阻塞的pipe, open()的O_NONBLOCK不行
  
  ulk:Nonblocking operations are usually requested by specifying the O_NONBLOCK flag in
  the open( ) system call. This method does not work for pipes, because they cannot be
  opened. A process can, however, require a nonblocking operation on a pipe by issuing a
  fcntl( ) system call on the corresponding file descriptor.
- 仅当pipe _没数据_ 且 写端的 _file object_ 没有被进程使用时, _read()_ 也会返回0.
** static ssize_t pipe_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
*** fs/pipe.c:
- 这个函数调用pipe_readv()
** static ssize_t pipe_readv(struct file *filp, const struct iovec *_iov, unsigned long nr_segs, loff_t *ppos)
*** fs/pipe.c:
- 主要是借ulk的表19-3来看代码.讲解以表的行的情况来讲比较好.
- pipe size = 0
  
  1. 若没有pipe size 为 _0_ ,且 _没有写者_,那么就会返回0.这正如ulk上说的,
     
     pipe_inode_info.writers :Flag for (or number of) writing processes
     #+BEGIN_EXAMPLE
       	ret = 0;
   	down(PIPE_SEM(*inode));
   	info = inode->i_pipe;
  ...
  2. 若pipe size 为0, 且 _读者_ 不能阻塞,
     
     但在有没有 _阻塞写者_ 的情况下,代码与19-3图是不一致的,
     
     代码是在 _有阻塞写者_ 的情况下是 _不会_ 返回EAGIN,
	...
	for (;;) {
     ...
     从上面的注释来看,通常是要休眠的,但是如果写休眠者是内核态下休眠,那么可以不违反POSIX的情
     况下等待.为什么是这样不违反POSIX呢?

  3. 当pipe size为0, 且可以阻塞, 且 _有_ 写者但 _没有_ 写者休眠,那么可以休眠.
     #+BEGIN_EXAMPLE
  	ret = 0;
	down(PIPE_SEM(*inode));
	info = inode->i_pipe;
  ...
  4. pipe size 为0, 且 _有_ 写者休眠,也是上面的代码,也可以休眠.
     
     要注意这种情况下无论读者是否可以阻塞,读者都要休眠.
     
  所以只有在 _没有_ 写者 _休眠_ 时才会判断读者的 _O_NONBLOCK_.这样实现是不是因为:写者休眠
  是因为没有多余的空间,所以当读者读取数据后就要空间,这时写者可以马上把为数据写到pipe
  buffer,很快读者可以很快又读到数据,所以就不需要休眠了.

- 接下来是表19-3的 pipe size 大于0, 但是小于所要读的数据.
  
  1. 没有写者的情况,
     
     当读完所有的数据的后,就退出函数,返回已读的字节数.
     ...
     #+BEGIN_EXAMPLE
			...
			if (chars > total_len)
				chars = total_len;
     #+END_EXAMPLE
     无论后面是否还有pipe buffer可读,一定会进入这个判断,因为读者会把所有pipe buffer的数据
     都读完,包括当前这个
     #+BEGIN_EXAMPLE
   			if (!buf->len) {
				buf->ops = NULL;
     ...
     说说这个do_wakeup的含义, 就是如果一个pipe buffer(页框)的数据全被读取了,那么就可以唤醒
     写者了,
     #+BEGIN_EXAMPLE
   		if (do_wakeup) {
  ...
  2. 如果是非阻塞的 _读_ 时,那么代码又和表格不同了, 
     
     如果有 _写_ 者在等待的话, 那么就会等待.而表格不是这样的.
     
     表格的意思是只要是阻塞的读, 就算有写者在等待空余的pipe buffer, 读者也不会等待.
     
     有一种这样的情况,若有一个写者把16个pipe buffer全都填充完了,但还有n个没有填充的数据,这
     时写者会休眠,之后有一个读者会读取超过16个pipe buffer外加n个的数据, 
     
     这时如果按表格的方式实现, 那么读者在读完16个pipe buffer的数据后就返回EAGAIN.
     
     而按照linux的实现就不会返回EAGAIN,而会读完所有想要的数据,且正常返回.
     
     还有一点要注意的就是表格那里只是阻塞的读,是不管写者是不是在休眠都是一样的操作,
     
     而代码不是样的,有写者但是写者没有休眠才会判断读者是不是可以阻塞.但是为什么要这样做呢?
     好像有点不合理,应该是合理的,因为pipe_inode_info里的写者等待队列是因为没有pipe buffer
     写等待的,而没有等待的写者,肯定是干其它的事情去了,而没有写pipe buffer,所以阻塞的读者是
     不可以等待这些没有休眠的写者的.

  3. 若是 _阻塞读_ , _有_ _没有休眠_ 的写者,读者不用等待.这也符合表格所写的
     进入这个判断
     #+BEGIN_EXAMPLE
   		int bufs = info->nrbufs;
		if (bufs) {
     #+END_EXAMPLE 
     若数据跨过一个pipe buffer就会进入下面的判断.
     #+BEGIN_EXAMPLE
   		if (bufs)	/* More to do? */
			continue;
     #+END_EXAMPLE 
     ...
     #+BEGIN_EXAMPLE
   		if (!PIPE_WAITING_WRITERS(*inode)) {
     #+END_EXAMPLE 
     因为执行到这里时读者已读取了部分数据,所以会进入下面的判断
     #+BEGIN_EXAMPLE
			if (ret)
				break;
     #+END_EXAMPLE 
     进入上面的判断后会退出循环,重点是它没有对读者是否可以阻塞进行判断,这是正确的,因为写者
     没有数据可写,所以读者没有必要等待.
     
     因为读完所有的pipe buffer数据而退出,所以一定是释放了pipe buffer的,所以do_wakeup一定被
     设置为1了,所以会进入下面的判断
     #+BEGIN_EXAMPLE
	...
	if (do_wakeup) {
     ...
     #+END_EXAMPLE 
     但是不是没有写者吗?为什么还进入上面的判断呢?

  4. 如果是 _阻塞_ 读者,且有写者在 _休眠_,那么当然是读者会休眠了.
- 如果是pipe size大于要读的数据, 那么无论什么情况下读者都不会休眠的.
- 现在有点明白info_inode_info.tmp_page是怎么处理的了, 是为了提高给pipe的写者分配页的速度的.
  
  当一个读者读完一个pipe buffer里所有数据时(就是把页框里最后一个有效的数据给读取了),设这个
  叫P,就会调用pipe_buffer.ops.release()方法来释tmp_page所指向的页,并把tmp_page重新赋以页P.
  
  当一个写者要分配一个页框来存放数据时,就会使用tmp_page所指向的页.

** static ssize_t pipe_write(struct file *filp, const char __user *buf, size_t count, loff_t *ppos)
*** fs/pipe.c:
- 直接调用pipe_writev()
** static ssize_t pipe_writev(struct file *filp, const struct iovec *_iov, unsigned long nr_segs, loff_t *ppos)
*** fs/pipe.c:
- 无论在什么情况下,只要该pipe没有读者,那么会退出, 并发送SIGPIPE信号
  #+BEGIN_EXAMPLE
  ...
  #+END_EXAMPLE
  pipe_readv()里是在pipe size为0,没有写者时会返回0.
- 计算出所要写的数据大小模上页大小的数据量, 之后判断这些数据能不能 _全部_ 放到当前 _最后_
  一个pipe buffer的 _剩余_ 这个空间里,若可以就放,否则不放,分一个新的页框来放.
  
  是否可以合并,除了判断最后一个pipe buffer是否有空间之外,还要看
  pipe_inode_info.ops.can_merge是否设置了.
  #+BEGIN_EXAMPLE
  ...
  #+END_EXAMPLE 
  判断是否够空间和can_merge是否设置
  #+BEGIN_EXAMPLE
		if (ops->can_merge && offset + chars <= PAGE_SIZE) {
  #+END_EXAMPLE
  
  所以,不一定是每个pipe buffer填充完之后才可以分配一个新的pipe buffer. 但是为什么会这样设
  计呢?
  
  pipe_readv(), pipe_writev()都在函数的开头获取pipe信号量,在最后再释放信号量.所以不会出现之
  间的 _竞争_ 和 内部的 _重入_ 问题.但是两个函数为了等待 读/写 者而会调用pipe_wait()先释放
  信号量后两休眠.但是这样也不会在访问pipe_inode_info时会竞争.
- 发现表格19-4和代码很不一样,完全对不上号.
- 能执行到for这里,那么只两种情况,
  
  1. 和最后一个pipe buffer合并了,但是还有数据,且这些数据大小是页的整数倍.

  2. 可能一开始的数据量是页的整数倍,或 因最后一个pipe buffer剩下的空间不能存放原数据量模去
     页大小后的数据量而没有合并.
     
  所以执行到for这里剩下的数据量不一定是页大小的整数.
- 当没有空闲的pipe buffer时且写者能等待时才会休眠,而不像表19-4所述那样.
- 分配一个新页都是从头开始存数据的.
  #+BEGIN_EXAMPLE
			buf->offset = 0;
  #+END_EXAMPLE 
- 为了提高执行效率,对pipe_inode_info.tmp_page使用了技巧. 若tmp_page指后一个页,那么在需要一
  个新页存放数据时使用tmp_page页就可以,若没有就分配,成功分配之后,使用了技巧,就是把新分配的
  页指针赋给tmp_page,为什么这样做呢?因为在接下来执行中可能出现并没有真正使用该页就退出循环
  了.
  ...
  #+END_EXAMPLE
  若没有退出循环,那么就置tmp_page为NULL
  #+BEGIN_EXAMPLE
			info->tmp_page = NULL;
  #+END_EXAMPLE 
** not function
*** 19.2
- 因进程不能打开一个已存在的pipe,使进程不能打开pipe和进程通信,所以引入了FIFO,即命名pipe.
- ulk:Each FIFO is much like a pipe: rather than owning disk blocks in the filesystems, an
  opened FIFO is associated with a kernel buffer that temporarily stores the data
  exchanged by two or more processes.
- ulk:Thanks to the disk inode, however, a FIFO can be accessed by every process, because
  the FIFO filename is included in the system's directory tree.
- FIFO的读/写也是用pipe_read()和pipe_write()实现的.有两点不同:
  
  FIFO的inode是出现在系统目录树的, 而不是在pipefs文件系统的.
  
  FIFO是双向的.
- 创建FIFO时用 _mknod()_, 要指定S_FIFO.
  
  POSIX有一个 _mkfifo()_, linux是用mknod()实现的.
- 打开一个fifo时, 调用

  read_inode() -> 发现是特殊文件(fifo也是)就调用
  
  init_special_inode() 它设置 i_fop 为def_fifo_fops.
  
  然后 file object 的 f_ops设为 i_fop.
  
  执行open方法时就是fifo_open()
- fifo的inode.i_pipe也是使用pipe_inode_info结构体.
** static int fifo_open(struct inode *inode, struct file *filp)
*** fs/fifo.c:
- 若inode.i_pipe为NULL,那么也分配一个pipe_inode_info.
  #+BEGIN_EXAMPLE
	if (!inode->i_pipe) {
		ret = -ENOMEM;
		if(!pipe_new(inode))
...
- file.f_op
  ...
  1. 如果是 _只读_ 的情况,
     ...
     file.f_op要设为 _read_fifo_fops_ .
  
     read_fifo_fops.read 设为 _pipe_read_ .
  
     read_fifo_fops.write 设为 _bad_pipe_w_ .
     
     bad_pipe_w返回EBADF
  2. 若是 _只写_ 的情况.
     
     file.f_op要设为 _write_fifo_fops_.
     
     write_fifo_fops.read 设为 _bad_pipe_r_.
     
     write_fifo_fops.write 设为 _pipe_write_.
  3. 若是 _读写_ 的情况.
     
     file.f_op要设为 _rdwr_fifo_fops_.
     
     rdwr_fifo_fops.read 为 _pipe_read_.
     
     rdwr_fifo_fops.write 为 _pipe_write_.
- POSIX.1说就算 _没有_ _写_ FIFO的进程, _只读_ _O_NONBLOCK_ 打开FIFO也没有错
  #+BEGIN_EXAMPLE
	/*
	 *  O_RDONLY
	 ...
	 */
  #+END_EXAMPLE
- pipe_inode_info.wait,这个等待队列里应该是可以放 _读者和写者_ 的.
  
  因为pipe_readv()和pipe_write()里都调用pipe_wait()把当前进程插入到这个队列里.
  
  但是会不会在等待队列里 _同时存在_ 读者和写者呢?
  
  应该不会的,
  
  假如没有pipe buffer可给写者填数据时, 那么写者会调用pipe_wait()进行休眠, 但是在调用
  pipe_waite()前会调用会调用wake_up_interruptible_sync() _同步_ 唤醒pipe_inode_info.wait里
  的 _所有_ 进程. 在pipe_inode_info.wait的等待不是互斥的, 所以会唤醒队列里的所有进程. 因为
  在pipe_wait()里会获取等待队列的自旋锁, 又因为wake_up_interruptible_sync()已获取了这个锁,
  所以一定是调用wake_up_interruptible()把所有的任务都移出队列之后才会调用pipe_wait()把当前
  进程加到队列里.但是现在又有一个问题:会不会在调用wake_up_interruptible_sync()之后到调用
  pipe_wait()之前这个时间窗口里有其它的读者或写者被插到队列里呢?不会的,因为inode.i_sem的原
  因,所以这个时间窗口是一个被inode.i_sem保护的临界区.那么pipe_inode_info.wait里是不是只会
  出现一个任务呢?而不会同时出现多个任务呢?应该是,因为在插入任务前一定会清队列,插入和删除之
  间不会竞争.
- 只读时有这段代码的意思是:因为该FIFO没有读者,该读者是唯一的读者,所以等待队列里有进程时一
  定是写者,所以不管三七二十一把它唤醒就是了,
  #+BEGIN_EXAMPLE
  		if (PIPE_READERS(*inode)++ == 0)
			wake_up_partner(inode);
  #+END_EXAMPLE
- 在只读时, O_NONBLOCK 是影响fifo_open() 是否阻塞的,
  
  如果是 _非阻塞_ 的, 就设置 _file.f_version_.
  
  如果是 _阻塞_ 的,就会调用wait_for_partner()来等待至少一个写者唤醒并把数据写入FIFO,
  
  wait_for_partner()循环调用pipe_wait(),若发现r_counter/w_counter有变化就退出循环.
  
  好像r_counter和w_counter是只增加的,没有减少的.如果不是这样的话,wait_for_partner()也不可
  以只判断r_counter/w_counter的变化而不判断是增还是减.
  
  r_counter的增加只在以 _读_ 方式 _打开_ fifo文件时.
  
  w_counter的增加只在以 _写_ 方式 _打开_ fifo文件时.
  
  当fifo没有 _写者_ 且 读者可以 _阻塞_, 那fifo_open()会一直等待直到有写者, 在
  wait_for_partner()发现有写者表示可以马上读到数据吗?不一定的,假设进程A调用open()打开一个
  没有写者的fifo来读数据,因为没有写者而等待,接着进程B调用open()打开同一个fifo来写数据,写者
  会增加w_counter并执行wake_up_partner()
  #+BEGIN_EXAMPLE
		PIPE_WCOUNTER(*inode)++;
		if (!PIPE_WRITERS(*inode)++)
			wake_up_partner(inode);
  #+END_EXAMPLE 
  这时进程A还没有等待进程B退出open()来写数据,就向fifo读数据了.而在pipe_readv()里又会因为没
  有数据而等待写者把数据写进fifo,那么open()里的等待是不是有点多余呢?

- 当 _只写_ 时, 若发现这是 _唯一_ 的写者, 那么会调用wake_up_partner(), 这和只读的原因是一
  样的.
  
- 当 _只写_ 时, 若没有读者,也会调用 wait_for_partner() 等待以读的方式打开fifo, 这点与只读
  也相似,
  
  但是有一点不同, 就是不会判断 _O_NONBLOCK_ , 也就是说 _O_NONBLOCK_ 对写者无效.

- 如果是 _读写_ 的,处理就会很简单, 
  
  若读写是唯一的话还是会调用wake_up_partner().
  

- 在从这个函数可以看出,在FIFO里, pipe_inode_info.readers/writers不总是1,会增加的, 以读/写
  打开一次这个fifo就会自加一次readers/writers.
** not function
*** 19.3
- system v IPC 是一套机制, 
  
  这套机制包括: _信号量_, _消息_, _共享内存_.
  
  IPC 资源 指 IPC信号量或IPC消息队列或IPC共享内存.
  
- _一个_ IPC资源可以被 _任何_ 进程使用,即使进程之间没有任何关系.
  
  ulk:An IPC resource may be used by every process, including those that do not share the
  ancestor that created the resource.

- IPC 资源都有一个 _IPC identifier_ 标识, 是 _32_ 位的数字, 在系统范围是 _唯一_ 的, 所以可
  不同进程可以用这个标识找到相应的IPC资源来和其它的进程通信.
- semget() 创建 IPC 信号量, msgget() 创建 IPC 消息, shmget() 创建 IPC 共享内存.
  
  这些函数主要是创建一个IPC 标识符, 若是已被创建,那么也返回这个标识符.
  
  它们的第一个参数指定了IPC的标识符, 如果是0(IPC_PRIVATE), 那么就会创建一个 _新的_, 且只能
  在有 _亲属_ 关系的进程之间使用来通信.
  
  ulk:One process issues a semget( ), msgget( ), or shmget( ) function by specifying
  IPC_PRIVATE as its IPC key. A new IPC resource is thus allocated, and the process can
  either communicate its IPC identifier to the other process in the application[ ] or fork
  the other process itself. This method ensures that the IPC resource cannot be used
  accidentally by other applications.
- _sem_ids_, _msg_ids_, _shm_ids_ 这些全局变量的类型是 _struct ipc_ids_.
  
  不是sem msg shm 共享一个struct ipc_ids.
- ipc sem要搞清 _struct kern_ipc_perm_, _struct sem_array_, _struct sem_, _struct
  sem_undo_, _struct sem_queue_.
- ipc 信号量比 _内核_ 信号量复杂和不同的原因:
  
  1. IPC信号量是多个信号量的集合,所以可以保护多个独立的共享的信号量. IPC信号量里的信号量叫
     primitive 信号量.
     
     IPC信号量和每个IPC信号量里的 primitive 信号量都是有上限的.

  2. 有undo信号量机制,在进程没有undo信号量时退出时会启动undo进程运行时的信号量操作.
- 与IPC 信号量相关的函数有 _semget()_, _semop()_, _semctl()_.
- http://www.eefocus.com/article/09-06/75167s.html
  
  _键_ 和 _识别号_ 指的是不同的东西。系统支持两种 _键_ ： _公有和私有_ 。如果键是 _公有_ 的，
  则系统中所有的进程通过权限检查后，均可以找到System V IPC 对象的 _识别号_ 。如果键是私有的，
  则键值为0，说明每个进程都可以用键值0建立一个专供其私用的对象。注意，对System V IPC对象的
  引用是通过识别号而不是通过键.
  
  是针对进程之间要共享一个临界资源而言的，信号量的初值为1。实际上，信号量作为资源计数器，它
  的初值可以是任何正整数，其初值不一定为0或1。另外，如果一个进程要先获得两个或多个的共享资
  源后才能执行的话，那么，相应地也需要多个信号量，而多个进程要分别获得 _多个临界资源后方能运
  行_ ，这就是 _信号量集合机制_, Linux 讨论的就是信号量集合问题。
  
  信号量集合用struct sem_array表示, 信号量用struct sem表示, sem_array.sem是一个struct sem的
  指针,指向一个数组.
** void __init sem_init (void)
*** ipc/sem.c:
- 这个函数是初始化sem_ids全局变量的.
- 调用ipc_init_ids(), 可见无论是信号量, 消息, 还是共享内存都是调用这个函数的来初始化的,不
  同的是第二个参数不同.
  
  第二个参数指的是资源的最大数量.信号量的是SEMMNI (128),
** void __init ipc_init_ids(struct ipc_ids* ids, int size)
*** ipc/util.c:
- 无论是信号量的,还是消息的, 还是共享内存的, 资源的最大值都不能超过IPCMNI (32768)
  #+BEGIN_EXAMPLE
	if(size > IPCMNI)
		size = IPCMNI;
  #+END_EXAMPLE
- max_id: Maximum slot index in use
- ipc.ids.seq_max初始化为INT_MAX可以分成多少分的IPCMNI
  #+BEGIN_EXAMPLE
...
- 最后是初始化ipc_ids.entries
  ...
  给ipc_ids.entries.p分配了size个struct kern_ipc_perm _指针_.
- 因为size是小于IPCMNI的, 且SEQ_MULTIPLIER是IPCMNI, 所以每一个seq里的id数都不小于size.
** asmlinkage long sys_semget (key_t key, int nsems, int semflg)
*** ipc/sem.c:
- 参数key是键值, nsems是创建的信号量集里信号的个数.
- semflg参数的取值：IPC_CREAT ：如果内核中没有新创建的信号量集合，则创建它。
  IPC_EXCL ：当与IPC_CREAT一起使用时，但信号量集合已经存在，则创建失败。
  
  如果IPC_CREAT单独使用，semget()为一个新建的集合返回标识号，或者返回具有相同键值的已存在集
  合的标识号。如果IPC_EXCL与IPC_CREAT一起使用，要么创建一个新的集合，要么对已存在的集合返
  回-1。IPC_EXCL单独是没有用的，
- 若key是IPC_PRIVATE,那么就调用newary(),
- 若key不是IPC_PRIVATE,那么就先判断key所表示的这个id是不是已经被使用了,调用ipc_findkey()判
  断,
  
  若没被使用又没设IPC_CREAT, 那么就返回错,若设了IPC_CREAT, 那么也是调用newary().
- 若该id的资源被使用了,且IPC_CREAT和IPC_EXCL _都_ 设置了, 那么就返回EEXIST.
  #+BEGIN_EXAMPLE
...
- 那么剩下的情况就是:id是已被使用的.
  ...
  这时参数nsems不能大于新建时的sem_array.sem_nsems
  
  调用ipcperms来处理打开一已存在的IPC 信号量的访问权限.
  
  若以上都通过了,那么就调用sem_buildid()
...
*** ipc/util.c:
- 这个权限检查有点特别,
- ulk:The mode bit mask includes six flags, which store the read and write access
  permissions for the resource's owner, the resource's group, and all other users. IPC
  access permissions are similar to file access permissions described in the section
  "Access Rights and File Mode" in Chapter 1, except that the Execute permission flag is
  not used.
  
  只有6个标志, 与文件的权限的差别就是没有执行权限, 但是这个函数代码没有特殊处理执行的权限,
  就是 S_IRUSR, S_IWUSR, S_IRGRP, S_IWGRP, S_IROTH, S_IWOTH.
- 权限的检查是这样的:
  
  若 _当前进程的_ _有效_ 用户是 信号量的 _创建用户_ 或 _拥有者_,那么 若原地权限没有
  S_IRUSR 的,那么传入的权限 _不_ 能有 _读_ 的权限(S_IRUSR, S_IRGRP, S_IROTH),
  写也是这样.
  
  若 _当前进程的_ _有效_ 用户 不是 信号量的 _创建用户_ 或 _拥有者_,但当前进程的有效组是信
  号量的创建组或拥有组, 那么处理原来权限的S_IRGRP和S_IWGRP和有效用户的一样.
...
*** ipc/util.c:
- 看ids里的key这个id的IPC信号有没有被使用.
  
  判断的标准是ipc_ids.entries.p数组里的某个kern_ipc_perm.key 是否等于key
- ipc_ids.max_id表示的是当前使用的最大的id.
  
** static int newary (key_t key, int nsems, int semflg)
*** ipc/sem.c:
- used_sems是指当前一共使用的primitive 信号量数,是一个全局变量
  
  sc_semmns是指最多可以使用的primitive信号量数, 是一个全局变量, 是SEMMSL(256).
  
  #+BEGIN_EXAMPLE
	if (used_sems + nsems > sc_semmns)
		return -ENOSPC;
  #+END_EXAMPLE 
- 在sem_init()里调用的ipc_init_ids()分配了struct ipc_ids,并初始化了.
  
  在这个函数里分配了struct sem_array,并初始化了.
  #+BEGIN_EXAMPLE
	...
	memset (sma, 0, size);
  #+END_EXAMPLE
- 这个函数不管semflg的IPC_CREATE和IPC_EXCL.
  
  semflg里的权限是对所有的primitive信号量起作用的, 不是每个primitive信号量有独立的权限.
  #+BEGIN_EXAMPLE
	sma->sem_perm.mode = (semflg & S_IRWXUGO);
  #+END_EXAMPLE 
  键也是对所有的primitive信号量而言的,
  #+BEGIN_EXAMPLE
	sma->sem_perm.key = key;
  #+END_EXAMPLE
- 调用ipc_addid()来初始化sem_array.sem_perm,并在ipc_ids.entries.p数组里找个空闲的位置放下.
- 接下来就是增加used_sems
  #+BEGIN_EXAMPLE
	used_sems += nsems;
  #+END_EXAMPLE
  可见 used_sems 是对于 primitive 来说的.
- 初始化sem_array.sem_base
  #+BEGIN_EXAMPLE
	sma->sem_base = (struct sem *) &sma[1];
  #+END_EXAMPLE 
  
  为什么要这样初始化呢? 因为分配是 struct sem 和 struct sem_array 的空间一起分配的
  #+BEGIN_EXAMPLE
	size = sizeof (*sma) + nsems * sizeof (struct sem);
  #+END_EXAMPLE
- 初始化sem_nsems和sem_ctime
  #+BEGIN_EXAMPLE
	sma->sem_nsems = nsems;
	sma->sem_ctime = get_seconds();
  #+END_EXAMPLE
- 最后调用sem_buildid(), 就是计算
  #+BEGIN_EXAMPLE
	return SEQ_MULTIPLIER*seq + id;
  #+END_EXAMPLE
  
  ipc_ids.seq在ipc_addid()里若函数成功是会自增的, 自增之前赋给kern_ipc_perm.seq在
  ipc_addid()里若函数成功是会自增的,所以到执行sem_buildid时已经和上一次调用newary()时不同
  了.
  
  且id是ipc_addid()返回的新建的sem_array.sem_perm的地址所存在sem_ids.entries.p数组的下标.
  
  所以新建一个struct sem_array之后,它的id一定是固定的.
  
  那么怎么对一个已存在的信号量获得信号量所在的ipc_ids.entries.p数组下标号呢?就是通过用key来
  调用ipc_findkey()得到的, 
  
  sys_semget()第一个参数传入的是key, 返回的是用key调用ipc_findkey()所得的下标加上
  SEQ_MULTIPLIER*seq. seq在每新建一个struct sem_array时会增加,会回绕, 所以有些信号量的seq
  会相等, 但是由于id是唯一的, 所以sys_semget()返回的值是对于不同的信号量而不同的.
  
  seq为什么要这样处理呢?ulk:To minimize the risk of incorrectly referencing the wrong
  resource, the kernel does not recycle IPC identifiers as soon as they become
  free. Instead, the IPC identifier assigned to a resource is almost always larger than
  the identifier assigned to the previously allocated resource of the same type.
  
  seq就是槽号.
- 所以调用完这个函数之后的结果是
  
  1. 分配了一个新的 struct sem_array, 一堆的 struct sem,

  2. 并初始化了 sem_array和sem_array.sem_perm的成员,

  3. 把sem_array.sem_perm的地址放到了sem_ids(ipc_ids类型).entries.p数组合适的下标.
** int ipc_addid(struct ipc_ids* ids, struct kern_ipc_perm* new, int size)
*** ipc/util.c:
- 调用grow_ary()来扩展IPC信号量的可达到的最大总数,具体的就是ipc_ids.entries.p数组的大小.
  
  grow_ary()里若第二个参数size不大于ipc_ids.entries.size的时候,返回值就是第二个参数.
  
  但是第二个参数传入的都是当前的ipc_ids.entries.p数组的最大值, 若想扩展ipc_ids.entries.p,
  就要用更大的值.
- 接丁来就是找出第一个为空的索引
  #+BEGIN_EXAMPLE
	for (id = 0; id < size; id++) {
  ...
  分配id时是尽量分配最小的.
- 增加ipc_ids.in_use
- ipc_ids.max_id的意思应该是当前使用的最大id号,
  #+BEGIN_EXAMPLE
	if (id > ids->max_id)
		ids->max_id = id;
  #+END_EXAMPLE
- kern_ipc_perm.cuid : creator user ID
  kern_ipc_perm.uid: owner user ID
  #+BEGIN_EXAMPLE

	new->cuid = new->uid = current->euid;
	new->gid = new->cgid = current->egid;
  #+END_EXAMPLE
- kern_ipc_perm.seq应该是这样的, 每用一个id, 就会增加1, 到ipc_ids.seq_max就会回绕到 0, 但
  不会自减.
  #+BEGIN_EXAMPLE
	...
	if(ids->seq > ids->seq_max)
		ids->seq = 0;
  #+END_EXAMPLE
- 所以这个函数就是在ipc_ids.entries.p数组里找了一个空的kern_ipc_perm,并初始化这个
  kern_ipc_perm,返回该kern_ipc_perm所在的下标,就是id.
** static int grow_ary(struct ipc_ids* ids, int newsize)
*** ipc/util.c:
- 参数newsize就是要扩展的大小.
- ipc_ids.entries.size要改成newsize
  #+BEGIN_EXAMPLE
	new->size = newsize;
  #+END_EXAMPLE 
- 根据newsize分配一个空间,并初始化ipc_id_ary.p数组,
  #+BEGIN_EXAMPLE
  ...
  ipc_ids.entries使用新的空间 并 释放旧的空间
  #+BEGIN_EXAMPLE
	old = ids->entries;
...
*** ipc/sem.c:
- 这个函数调用sys_semtimedop()
** asmlinkage long sys_semtimedop(int semid, struct sembuf __user *tsops, unsigned nsops, const struct timespec __user *timeout)
*** ipc/sem.c:
- http://baike.baidu.com/link?url=ZisHBwW2F12WfK6hUGsoXwlA24FqLRyM2eFymZSzv5sbFTepGPni3X7ryCUyHjQQ7SoqJzf6tZNEoflkGvbwNa
  #+BEGIN_EXAMPLE
...
struct sembuf {
  ...
  sem_num是相对应的信号量集中的某一个资源,所以其值是一个从0到相应的信号量集的资源总数
  （ipc_perm.sem_nsems）之间的整数.
  
  sem_op指明所要执行的操作,
  
  sem_flg说明函数semop的行为.sem_op的值是一个整数,如表2所示,列出了详细sem_op的值及所对应的
  操作.
  ...
  2. 0 
     ...
     - sem_flg没有指定IPC_NOWAIT,则将该信号量的semncnt值加1,然后进程挂起直到下述情况发生。
       ...
       - 此信号量被删除（只有超级用户或创建用户进程拥有此权限）,函数smeop出错返回EIDRM；
       - 进程捕捉到信号,并从信号处理函数返回,在此情况将此信号量的semncnt值减1,函数semop出错返回EINTR
  3. 负数 
      
     - 请求sem_op的绝对值的资源。如果相应的资源数可以满足请求，则将该信号量的值减去sem_op的绝对值，函数成功返回。
     - 当相应的资源数不能满足请求时，这个操作与sem_flg有关。
       - sem_flg指定IPC_NOWAIT，则semop函数出错返回EAGAIN。
       - sem_flg没有指定IPC_NOWAIT，则将该信号量的semncnt值加1，然后进程挂起直到下述情况发生：
         - 当相应的资源数可以满足请求，该信号的值减去sem_op的绝对值。成功返回；
         - 此信号量被删除（只有超级用户或创建用户进程拥有此权限），函数smeop出错返回EIDRM：
         - 进程捕捉到信号，并从信号处理函数返回，在此情况将此信号量的semncnt值减1，函数semop出错返回EINTR 
- 参数nsops是指定参数tsops数组的元素个数
- 因为tsops是在用户态空间的, 所以要把它拷贝到内核态空间.
  
  又因为tsops是一个数组,所以拷贝之前要有一个可以容纳tsops数组大小的空间.
  
  这个内核空间可以是用堆栈分配优先, 就是以局部变量的方式, 这种方式只能在执行前指定大小,
  #+BEGIN_EXAMPLE
  ...
  在栈分配比在堆分配快.
- 调用find_undo()在当前进程的sysvsem里找sem_undo.semid等于semid的sem_undo.
- 对IPC信号量进行操作,就要检查权限
  #+BEGIN_EXAMPLE
  ...
  所以,当参数tsops数组里有要对信号量作加减操作的,那么信号量的kern_ipc_perm.sem_perm就要有
  写的访问权限,否则就要有读的权限,
  
  即没有写权限,也没有读权限也是不行的.
- 调用try_atomic_semop()让sops的所有sembuf作用于对应的sem_array.sem, 若成功就返回0, 若要休
  眠就返回1. 有错(信号数超范围了)就返回错(负数).
  #+BEGIN_EXAMPLE
  ...
  从代码可以看出不用等待就可以返回了,否则下面还要做等待队列的处理
- 若try_atomic_semop()返回0表示所有的sembuf都已作用于对应的sem_array.sem.
  
  所以这样可能可以使一些等待信号的进程可以被唤醒.
...
- 接下来就是要做休眠的处理了.
  ...
  把参数sops, nsops, id分别保存到sem_queue.sops, sem_queue.nsops, sem_queue.id.
  
  把current对应这个IPC信号量的sem_undo保存到sem_queue.undo, 因为到时处理这个sem_queue的时
  候要处理对应的sem_undo.
  
  sem_queue.alter是在update_queue()时用来判断处理完这个sem_queue时是否要从头扫描
  task_struct.sysvsem链表.
  
  sem_queue.alter在sops数组里有sembuf的sem_op非0时就会设为1
  #+BEGIN_EXAMPLE
  ...
  所以sem_queue.alter在sem_queue.sops数组里有sembuf要对primitive进行加减操作时就设为1
  
  若sem_queue.alter为1, 那么就把sem_queue加在链表的后面, 否则就加在前面以让这个sem_queue最
  快处理, 因为处理这样的sem_queue消耗primitive信号量.
- 这个函数的参数timeout,指的是在需要阻塞时调度的超时时间
  #+BEGIN_EXAMPLE
	...
	if (timeout)
		...
		schedule();
  #+END_EXAMPLE
- 调度回来以后就判断sem_queue.status的状态, 若是IN_WAKEUP,表示该sem_queue已被另一个进程从链
  表里删除了,但是那个进程还没调用wake_up_process()唤醒该进程,这可以从update_queue()里了解,
  
  所以这时要占着cpu等待,但又不禁止中断,就是类似用死循环,直到sem_queue.status不等于
  IN_WAKEUP.
  #+BEGIN_EXAMPLE
  ...
  #+END_EXAMPLE
- 这个函数在等待时只在3种情况会返回,之前已说过, 
  
  信号量数满足; IPC信号量被删除; 接收到信号.
  
  接下来的代码就是处理这3种情况的.
  
  如果没有被删除,也没有收到信号,schedule()返回后一定是其它的进程因信号量满足而在
  update_queue()调用wake_up_process()来唤醒进程的吗?
  
** static inline int get_undo_list(struct sem_undo_list **undo_listp)
*** ipc/sem.c:
- IPC信号量的undo机制使用了两个链表, 一个是per-process, 另一个是per-semaphore, 
  
  task_struct.sysvsem_list是和 per-process链表有关的, ulk:The per-process list includes
  all sem_undo data structures corresponding to IPC semaphores on which the process has
  performed undoable operations. The sysvsem.undo_list field of the process descriptor
  points to a data structure, of type sem_undo_list , which in turn contains a pointer to
  the first element of the list; the proc_next field of each sem_undo data structure
  points to the next element in the list.

- 这个函数的作用很简单, 就是通过undo_listp这个参数返回current.sysvsem.undo_list.
  
  若current.sysvsem.undo_list.为空, 那么就分配一个
** static struct sem_undo *find_undo(int semid)
*** ipc/sem.c:
- 这个函数一开始就用get_undo_list(),上面有介绍.
- 再调用lookup_undo()在进程的sysvsem链表里找出sem_undo.semid等于参数semid的struct sem_undo.

  若找到就返回这个sem_undo, 否则就新建一个.
  ...
  因为是在解锁的情况下新建的, 所以在插入链表之前要加上锁后检查是不是已被其它的进程给插入了,若
  是就返回.
  #+BEGIN_EXAMPLE
	lock_semundo();
  ...
  kern_ipc_perm.deleted原来是这样用的.
- 关于struct sem_undo的四个成员的初始化

  sem_undo.proc_next就插到进程的sysvsem链表里
  #+BEGIN_EXAMPLE
	new->proc_next = ulp->proc_list;
	ulp->proc_list = new;
  #+END_EXAMPLE 
  
  sem_undo.id_next就插在ID为semid的这个信号量的sem_array.undo这个链表里
  #+BEGIN_EXAMPLE
	new->id_next = sma->undo;
	sma->undo = new;
  #+END_EXAMPLE 
  
  sem_undo.semadj指向一个大小为IPC信号量的primitive 信号量个数的数组空间
  #+BEGIN_EXAMPLE
	memset(new, 0, sizeof(struct sem_undo) + sizeof(short)*nsems);
	new->semadj = (short *) &new[1];
  #+END_EXAMPLE 
  
  sem_undo.semid就是该sem_undo所属的IPC信号量的ID号.
  #+BEGIN_EXAMPLE
	new->semid = semid;
  #+END_EXAMPLE
- 所以这个函数就是:若进程的sysvsem为空,那么就建一个,若进程的sysvsem链表里没有semid这个ID的
  信号量的sem_undo就新建一个.  
** static struct sem_undo *lookup_undo(struct sem_undo_list *ulp, int semid)
*** ipc/sem.c:
- 参数semid是在semget()一直传下来的信号量ID.
- 这个函数的作用就用在传入的struct sem_undo_list链表里找出sem_undo.semid等于参数semid.
** static int try_atomic_semop (struct sem_array * sma, struct sembuf * sops,int nsops, struct sem_undo *un, int pid)
*** ipc/sem.c:
- 若这个函数返回1,表示参数sops里至少有一个对IPC信号量的操作是会被阻塞的,还有一个条件就是要
  清IPC_NOWAIT才会返回1.
- 对在sops数组里的操作一个一个检查看会不会引起休眠.
  #+BEGIN_EXAMPLE
	for (sop = sops; sop < sops + nsops; sop++) {
  ...
  1. 当sembuf.sem_op为 _0_ 时,若sem_array.sem_base[sembuf.sem_num].semval不为 _0_,就要根据
     sembuf.sem_flg的IPC_NOWAIT来确定是否等待.
  
     注意是semval0与非0的情况,不管是不是正数.
     #+BEGIN_EXAMPLE
		curr = sma->sem_base + sop->sem_num;
		sem_op = sop->sem_op;
		result = curr->semval;
		...
		if (!sem_op && result)
			goto would_block;
     #+END_EXAMPLE 
  2. 当sembuf.sem_op为 _负数_ ,且sembuf.sem_op加个sem_array.sem_base[sembuf.sem_num].semval为
     _负数_
     ...
     #+BEGIN_EXAMPLE
		...
		if (result < 0)
			goto would_block;
     #+END_EXAMPLE 
- 若有sembuf.sem_flg的SEM_UNDO设置了,那么就把undo时的调整值放到 _当前进程_ 对于 _这个IPC信
  号量_ 的sem_undo.semadj[sembuf.sem_num]里去, 调整值的针对primitive信号量的.
  
  调整值不是把没修改前的sem.semval值存到sem_undo.semadj[sembuf.sem_num]里去,而是
  sem_undo.semadj[sembuf.sem_num]-sembuf.sem_op.
  #+BEGIN_EXAMPLE
		if (sop->sem_flg & SEM_UNDO)
			un->semadj[sop->sem_num] -= sop->sem_op;
  #+END_EXAMPLE
- 信号量的值超范围有两种情况:
  1. sem.semval加上sembuf.sem_op时超范围
     #+BEGIN_EXAMPLE
   		if (result > SEMVMX)
			goto out_of_range;
     #+END_EXAMPLE 
  2. sem_undo.semadj[sembuf.sem_num]减去sembuf.sem_op时超出范围
     #+BEGIN_EXAMPLE
   			int undo = un->semadj[sop->sem_num] - sem_op;
			/*
	 		 *	Exceeding the undo range is an error.
			...
			if (undo < (-SEMAEM - 1) || undo > SEMAEM)
				goto out_of_range;
     #+END_EXAMPLE 
- 这个函数成功后, 参数sops数组的所有元素都作用到了相应的的sem里去了, SEM_UNDO也处理了.
  
  若不成功,就是至少有一个元素不是阻塞就是超范围, 且所有的元素都不能作用于sem里去.
** static void update_queue (struct sem_array * sma)
*** ipc/sem.c:
- 一个struct sem_queue是对应一个休眠进程的.
  
  等待队列不是一个双向循环链表, 而是一个双向链表.所以分别有sem_array.sem_pending和
  sem_array.sem_pending_last指向第一个结点和最后一个结点.这样做是为了像FIFO,插入时放在最后.
  
  一个进程要对一个IPC信号量的多个primitive信号量处理, 但是又会因为有其中一个primitive不满
  足就会休眠, 所以在sem_queue里有一个有struct sembuf数组, 就是sem_queue.sembuf了.
  
  只要有一个sem_undo就可以了,因为sem_undo.semadj是一个包含了所有primitive信号量的数组.

- 这个函数对IPC信号量的sem_pending链表的sem_queue都扫描一遍,
- 对每个sem_queue都调用try_atomic_semop()
  #+BEGIN_EXAMPLE
		error = try_atomic_semop(sma, q->sops, q->nsops,
					 q->undo, q->pid);
  #+END_EXAMPLE 
  
  若sem_queue.sembuf数组里的元素都满足了就会返回0.
- 若sem_queue里所有的primitive都满足了,那么就把该sem_queue从sem_array.sem_pending链表里删
  除.
  #+BEGIN_EXAMPLE
			remove_from_queue(sma,q);
  #+END_EXAMPLE
- 在准备唤醒之前要设sem_queue.status为IN_WAKEUP,为什么要这么做呢?
  #+BEGIN_EXAMPLE
			q->status = IN_WAKEUP;
  #+END_EXAMPLE 
  在调用wake_up_process()之后要设status为try_atomic_semop()的返回值, 那么这时status只有两
  个值, 一个是0, 表示sem_queue的所有primitive请求都满足, 另一个是ERANGE,表示请求是非正常范
  围.对于这两种情况,按逻辑都应该要唤醒进程的.
- 若对sem_queue的操作会时影响整个链表,那么这个sem_queue就alter成员就设为1.
  
  这时就要重新扫描链表,否则就是扫描下一个,
  #+BEGIN_EXAMPLE
			/*
			...
			if (q->alter)
				...
				n = q->next;
  #+END_EXAMPLE
- 所以可见, 一个进程不是对单个primitive信号量操作的, 而是对一整个IPC信号量操作的. 就是说若
  需要一个IPC信号量里的两个primitive信号量时,不能先获取其中的一个primitive信号量,后发现不
  能获取另一个primitive信号量而休眠.
** not function
*** 19.3.4 IPC messages
- 一个消息由 _固定大小的头_ 和 _可变长度的文本_ 组成. 
  
  不同的消息由一个整型值来标签, 就是 _message type_. 通过这个message type进程可以直接获取
  指定的消息.
  
  ulk:As we'll see, the message queue is implemented by means of a linked list. Because
  messages can be retrieved in an order different from "first in, first out," the name
  "message queue" is not appropriate. However, new messages are always put at the end of
  the linked list.

- IPC message queue的个数默认是16, 
  
  消息大小默认是8192个字节.
  
  _一个_ 队列里的所有消息大小的总和默认是16384字节.

- 与sem_ids相似, 消息队列也有一个msg_ids.
  
  与IPC 信号量一样, 消息队列有一与sem_array类似的msg_queue. msg_queue也有一个类型为
  kern_ipc_perm.

- 一个消息队列里的所有消息被链到msg_queue.q_messages队列里.

- 整个消息至少有一个页的大小, 第一个页的开头是类型为msg_msg的消息头, 消息文本紧跟在消息头
  的面后
  
  msg_msg.m_list链起前后的消息, 
  
  消息文本不用使用东西来标明, 因为消息文本是紧跟在消息头的后面的.若文本超过4072个字节的话,
  就是说消息头加上消息文本时已超过一个页的大小,那么msg_msg.next就会指向第二个页的
  msg_msgseg结构体,除了第一个页,其它的页开头是一个只有next成员的msg_msgseg, 这个成员是指向
  下一个页的.

- 当一个进程把一个消息插入到一个已满的消息队列时就会被阻塞, 并把进程放到 _msg.q_senders_ 这
  个等待队列里

- 一个消息队列若没有进程指定要接收的消息(消息类型), 那么这个进程会被阻塞, 进程放到
  _msg.q_receiver_ .
** asmlinkage long sys_msgget (key_t key, int msgflg)
*** ipc/msg.c:
- 这个函数和sys_semget()类似, 不是用newary(),而是用newque()
** asmlinkage long sys_msgsnd (int msqid, struct msgbuf __user *msgp, size_t msgsz, int msgflg)
*** ipc/msg.c:
- 调用msg_load()来把用户态的文本消息放到struct msg_msg里.
- 发送消息队列要有写权限才可以
  #+BEGIN_EXAMPLE
		...
		if (ipcperms(&msq->q_perm, S_IWUGO))
			goto out_unlock_free;
  #+END_EXAMPLE
- 除了所有的消息文本的大小不能超过最大值之外,为什么消息数也不能超过最大的字节数呢?难道是因
  为过多的空消息(文本字节为0)而占过多的内存空间.
  #+BEGIN_EXAMPLE
...
- 若要等待, 那么就把进程加到送发的等待队列里去.
  #+BEGIN_EXAMPLE
		...
		ss_del(&s);
  #+END_EXAMPLE 
  在上面的代码可以看出,因为删除一个消息时一定会同时把它从等待队列里删除,所以若发现消息被删
  了,就不用再调用ss_del()了.
  
  这个函数是使用死循环来等待的.
- 成功把一个消息插入队列之后就调用pipelined_send(),pipelined_send()函数若发现所插入的消息
  正有一个接收者在等,那么唤醒它. 若在pipelined_send()里把所有等待该消息的进程都唤醒了, 那
  么就返回0.
  
  为什么这个消息没有等待的进程(就是把所有的接收者都唤醒了也不能把消息的文本全读走)才会插入
  到msg_queue.q_messages.
  
  
** struct msg_msg *load_msg(const void __user *src, int len)
*** ipc/msgutil.c:
- 这个函数是把在src里的数据转成一个消息的文本.
- 首先分配整个消息的长度,包括消息头和消息文本的空间,所以可能有多个连续的页
  #+BEGIN_EXAMPLE
...
- 然后把第一个页的空间填满
  ...
  #+END_EXAMPLE 
  copy_from_user()只能对一个页操作.
  
  调用完后alen就是剩下没有拷贝的大小,
- 如何把地址定位到下一个页呢?就是在源地址加上已拷贝的字节数
  #+BEGIN_EXAMPLE
...
*** ipc/msg.c:
- 扫描参数msq.q_receivers里的进程是否是在等待接收参数msg这个消息.若是就唤醒它.
- 通过调用testmsg()来判断所等待的消息就是传入的消息.
- 在这个函数可以看出,若发现有进程要求接收的消息大小(msg_receiver.r_maxsize) _小于_ 参数msg
  消息的大小, 就会继续处理 _下一个_ 接收者.
  
  若发现有 _大于_ 的,那么就会退出函数.
- 这个函数若扫描完了所有的接收者, 那么就说明所有的接收者都被唤醒过了, 这时会返回0
- 在请求的消息小于消息自身的大小时为什么设置msg_receiver.r_msg呢?从代码来看好像没有什么用
  啊.
  #+BEGIN_EXAMPLE
...
*** ipc/msg.c:
- SEARCH_ANY: _任何_ 的类型都符合.
  
  SEARCH_LESSQUEAL: _小于或等于_ 的类型符合.
  
  SEARCH_EQUAL: _等于_ 的类型符合.
  
  SEARCH_NOTEQUAL: _不等于_ 的.


** struct sk_buff *alloc_skb(unsigned int size, int gfp_mask)
*** net/core/skbuff.c:
- size是存放数据的空间大小
- 先分配sk_buff结构, 但不用__GFP_DMA分配
- 要求size的对齐后的大小
  #+BEGIN_EXAMPLE
...
\#define SKB_DATA_ALIGN(X)	(((X) + (SMP_CACHE_BYTES - 1)) & \
  ...
  不满一个SMP_CACHE_BYTES的就算一个,满一个但不满两个的就算两个.
- skb.truesize 是整个skb的大小,包括数据缓存区,还有之间的一个空白区
  #+BEGIN_EXAMPLE
	skb->head = data;
	skb->data = data;
	skb->tail = data;
	skb->end  = data + size;
  #+END_EXAMPLE
- 紧跟在数据buffer的是struct skb_shared_info.所以skb->end指向skb_shared_info的开头
- 所以这个函数的任务就是分配3个东西:数据buffer, struct skb_buff,struct skb_shared_info.最后
  还初化skb_buff的一些成员.
** static inline void skb_reserve(struct sk_buff *skb, unsigned int len)
*** include/linux/skbuff.h:
- 就是skb.data到skb.tail这一段整体往后移.
...
*** include/linux/skbuff.h:
- 这个是把skb->tail往后移,同时还更新了skb->len
** static inline unsigned char *skb_pull(struct sk_buff *skb, unsigned int len)
*** include/linux/skbuff.h:
- 这个函数是把skb->head向skb->tail方向移动.
** void __kfree_skb(struct sk_buff *skb)
*** net/core/skbuff.c:
- 释放skb->dst
- 如果初始化了skb->destructor函数就调用它
- 之后调用kfree_skbmem()释放主buffer和ip段的空间
** void skb_release_data(struct sk_buff *skb)
*** net/core/skbuff.c:
- 若skb_buff没有被克隆那么就会释放所有的段和主buffer
...
- 若在被克隆，但是引用计数为0,那么也释放所有的段和主buffer
#+BEGIN_EXAMPLE
	if (!skb->cloned ||
...
#+END_EXAMPLE
  ...
  skb_buff克隆只是多一个skb_buff结构体的副本, 这时主buffer的内容是不能被修改的, 这时
  skb->clone设成1, skb->dataref加1, 克隆的skb.users为1

  不明白为什么若没被克隆时就不用管引用计数而释放主buffer和段呢?

  skb->clone只设为1,不能设为2,那如果有多个克隆怎么识别呢?

  发现一点:skb_share_info->dataref只有skb_clone()这个函数加1,而且只有在这个函数里才被减.

  发现一点:是否决定删除主buffer和段是与skb->users没有关系的,这个说法是错误的,在kfree_skb()
  函数里做了检查,若为1就可以调用__kfree_skb (),然后再调用这个函数.所以再结合
  skb_share_info->dataref只在skb_clone()里增加,所以可以知道只有在skb->users为1是才会调用这
  个函数,但skb->users为1时可能会被克隆,因为被克隆之后,是不会知道谁是主体,它们之间是独立
  的,skb->users, skb_share_info->dataref, skb->clone这些都是独立的.

  如果skb->users为1就一定会删除skb_buff, 因为__kfree_skb()里直接调用kmem_cache_free()释放
  skb_buff.但是是不是要删除主buffer和段,就要先判断是不是有其它的skb_buff在引用它, 怎么判断
  呢?通过skb->clone(注意是先判断主buffer和段是不是能删除再释放skb_buff)来判断,若没有克隆(若
  有被克隆,那么所有的skb_buff都会clone都设成1),那么就直接释放主buffer和段,因为没有被克隆
  skb_share_info->dataref就一定为1,若有被克隆,那么就减skb_share_info->dataref,为0就释放.

  好像skb->clone和skb_share_info->dataref是有重叠的,后者已经完全可以说明前者了.

** static irqreturn_t vortex_interrupt(int irq, void *dev_id, struct pt_regs *regs)
*** drivers/net/3c59x.c:
- 很多的网卡的中断处理程序的实现都有相似模板.这一个其中一个网卡中断处理程序的实现.
- 设备可以为不同的原因发一个中断,然后可以通过设备的寄存器来辨别.
- 在处理的过程虽然禁止了中断,但是设备自己还是可以自已设置相应的中断标志以表示又有新的中断产
  生.但处理中断的个数还是有限的.
- 发现这样的一个函数调用:
  #+BEGIN_EXAMPLE
  	(vp->full_bus_master_rx ? boomerang_interrupt:vortex_interrupt)(dev->irq,dev,NULL);
  #+END_EXAMPLE
- 如果是接收中断的话,会调用vortex_rx()转而调用netif_rx ().
