...
1. next, prev, rq
2. preempt_disable(), this_rq()
3. check kernel lock
4. get run_time, sched_clock()-prev->timestamp
5. limit in 1s
...
7. PF\_DEAD
   ...
   :END:
8. not in running stat and not be preempt in kernel mode then remove from rq

   :PROPERTIES:
  why remove from rq not active list, and where is also remove it from rq
  :END:
9. TASK_INTERRUPTIBLE(no TASK_STOPPED) and not pending by signal then
