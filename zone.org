...
scp 是没有-f这个参数的,加上这个参数就会一直卡在哪里的.

IT好处:不工作可以学习.所以离职后可以边放松边学习.


==============================[2014-05-17 六]==============================

加了一些emacs dired的一些功能:如可以用s s来按大小排序.
http://www.newsmth.net/pc/pccon.php?id=2525&nid=58509


==============================[2014-05-20 二]==============================

如何使用emacs+cscope看remote服务器的代码,emacs是本地的,cscope是remote服务器的,要修改
xcscope.el文件:把start-process替换为start-file-process, call-process替换为process-file. 这
个修改是根据这里的:http://lists.gnu.org/archive/html/help-gnu-emacs/2009-02/msg00372.html.
cscope.out, cscope.po.out, cscope.in.out,这些文件是使用remote服务器的cscope生成的.使用这种
方法还可以绕过一些加密系统.就是说本不用使用加密客户端也可以.


叫我审代码,几千行的代码怎么审啊,我就只能找我最熟悉的那一段来审,其它的看一看格式就算了,这是一个好的方法.

==============================[2014-05-21 三]==============================

这个可以看某个进程的所有打开的文件描述符
 ls -l /proc/14926/fd

不能简单通过共享内存把文件描述符传给其它进程

每一个进程在内核空间都有一个打开文件的数组，这个文件描述符的整数值就是用来索引这个数组的，
而且，这个文件描述符只是在本进程内有效，也就是说，在不同的进程中，相同的文件描述符的值，代
表的可能是不同的打开文件。因此，在进程间传输文件描述符时，不能简要地把一个文件描述符从一个
进程传给另外一个进程，中间必须做一过转换，使得这个文件描述在目标进程中是有效的，并且它和源
进程的文件描述符所对应的打开文件是一致的，这样才能保证共享。
...
面试问题:上班时间(大小周),加班情况,leader或直接主管(分配任务的人)技术如何,研发人员的平均年
龄是多少,我负责的具体工作是什么(不清楚就又是打杂了),还要核对一下招聘信息所说的职责.

按下TAB 再按C-x b切换buffer会高亮

今天编译内核的时候发现每一次编译都会把代码根目录的cscope文件给删除掉, 所以在编译之后加一句
话把副本给拷贝过去.

==============================[2014-06-05 四]==============================

今天在用emacs远程cscope的时候,因为还没有建cscope文件就输入了c-c s g的命令,导致就算建了所有
的cscope文件后,emacs还是不能用c-c s g之类的命令,一用就卡在那里,要关掉emacs再开才可以.

内核态和用户态的交互可以用NETLINK
socket(PF_NETLINK, SOCK_DGRAM, NETLINK_KOBJECT_UEVENT);
http://www.cnblogs.com/iceocean/articles/1594195.html

==============================[2014-06-07 六]==============================

主流驱动:base, bluetooth, char(里面很杂), clk, clocksource, dma, gpio, i2c, input,
iommu, mmc, mtd, power, rtc, spi, usb, tty/serial

base/core.c bus.c platform,c class.c driver.c

设备插入是如何被系统认别的.
http://bbs.csdn.net/topics/390636313?page=1
